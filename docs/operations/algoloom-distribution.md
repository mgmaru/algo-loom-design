...
| fixture | テストや検証のために用意する、内容または振る舞いが固定されたもの。架空データ等の**固定データ**と、本物の代わりに置いて同じように振る舞う**代用部品**の両方を指す。AlgoLoomの配布物に含める架空データは前者で、審査担当者へ渡す代用ヘルパーは後者 |
...
GitHub Releasesへ実行ファイルを置く場合、利用者がブラウザーでダウンロードするとmacOSのquarantine属性とWindowsのMark-of-the-Webが付き、Developer ID署名・notarizationまたはAuthenticode証明書が必要になります。AtCoder認証は[AtCoder認証設計 §3.6.5](../architecture/atcoder-authentication.md#365-契約)で、有償の署名制度に依存する独立配布物を作らないことを契約としています。ブラウザーでダウンロードさせる配布形態を追加する場合は、認証ヘルパーをそこへ含めず、AlgoLoom全体の配布形態の変更として設計へ戻します。Homebrew tapとハッシュ照合を伴う`curl`取得にはこの制約はかかりません。

### 9.3. AtCoder認証用Chrome拡張機能

AtCoder認証の方式Aは、AlgoLoom本体と**認証用Chrome拡張機能**の2つで成立します。この拡張機能はPythonパッケージへ同梱できず、別の経路で配布します。

| 配布物 | 配布経路 | 利用者の操作 |
|---|---|---|
| AlgoLoom本体（認証ヘルパーを含む） | PyPI（`pipx`、`uv tool`、`pip`） | 導入1回 |
| 認証用Chrome拡張機能 | Chrome Web Store | 初回だけ標準追加画面で追加を承認する |

Chromeは一般利用者向けにChrome Web Store経由の追加しか受け付けないため、拡張機能を`pip`、`curl`、Homebrew、GitHub Releasesで配ることはできません。デベロッパーモードと手動読込は、利用者へ開発者向け設定を要求するため採用しません。

利用者がChrome Web Storeを自分で検索する必要はありません。`aloom auth login`が掲載ページを開くところまで行います。詳細と、実行ファイル署名が必要になる条件は[AtCoder認証設計 §3.6](../architecture/atcoder-authentication.md#36-配布物と実行ファイル署名の境界)を正本とします。

この拡張機能の公開範囲、審査、公開停止の手順は、`V-12`の成立を確認してから配布手順へ組み込みます。現時点で一般公開の手順は確定していません。

拡張機能の版と可用性をAlgoLoomが制御できないことによる運用リスクは、[Chrome拡張機能配布の運用リスクと緩和策](../project/chrome-extension-distribution-risks.md)を参照します。特に、掲載を取り下げるとインストール済みの利用者へChromeが警告を表示するため、配布停止手順を利用者の有無で区別する必要があります。
...
- 認証用Chrome拡張機能をChrome Web Storeへ限定公開し、開発者向け設定なしで標準追加画面から追加できることを実機で確認する（[§9.3](#93-atcoder認証用chrome拡張機能)）。
...
- 認証用Chrome拡張機能の公開、審査、公開停止の手順をリリース手順へ含める。PyPIのリリースと拡張機能の審査は所要時間が異なり、AlgoLoomは審査済みの拡張機能の版を固定して照合するため、**拡張機能の審査通過を待ってから、その版を要求するPyPI版を公開する**順序にする。詳細な互換性規則は`TD-38`で決める。
- 非公式表示を明記する。
