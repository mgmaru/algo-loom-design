...
> 確認日: 2026年9月20日
>
> 状態: `TD-37`と`TD-39`は完了。現在は`TD-42`が進行中で、**修正版`0.1.1`が審査に合格し（2026年9月20日。[§8.1.7](#817-2026年9月20日の011審査通過記録)）、同日に明示承認を得て限定公開した**（[§8.1.8](#818-2026年9月20日の011限定公開記録)）。**current versionは`0.1.1`で、配信bytesとsourceの照合まで完了した**（[§8.1.9](#819-2026年9月20日の011配信bytes取得記録)）。**`TD-42`は完了し、`TD-11`の保留が解けた。** deferred publishingの2026年10月19日期限は消化済み。Distribution設定は料金なし・`Unlisted`・日本のみ。更新test用`0.1.2`は未upload。account名、実address、developer dashboard URLは記録していない
>
...
3. 審査への提出（`0.1.0`は2026年8月27日完了、合格の確認は2026年9月18日。修正版`0.1.1`は2026年9月19日完了、合格の確認は2026年9月20日）
4. 審査通過後の`Unlisted`公開（`0.1.0`は2026年9月18日完了。`0.1.1`は2026年9月20日完了。いずれも別の明示承認を得て実行した）
5. 更新test用`0.1.2`のupload・審査提出・反映（未実施。[ADR-0005](../../decisions/0005-verify-consent-flow-in-browser-semantics.md)で`0.1.1`を修正版へ充てたため、更新testに使う版は`0.1.2`へ繰り下げた）
6. campaign終了後にitemを非公開化する操作

一つの承認で複数を許可する場合も、対象item、版、支払画面に表示された金額・通貨・税、`Unlisted`の意味、停止方法、実行者を承認記録へ明記します。「`TD-39`を進めてよい」「`TD-42`を進めてよい」という一般的な依頼を、問い合わせ送信、契約、支払い、審査提出または公開の承認へ読み替えません。
...
2026年9月20日時点で、次まで完了しています。日付は同じですが、この節は限定公開の実行後に読み直したものです。
...
- **`0.1.0`の審査への合格**（2026年9月18日にownerが確認。[§8.1.2](#812-2026年9月18日の審査通過記録)）
- **明示承認に基づく`0.1.0`の`Unlisted`公開**（2026年9月18日にownerが実行。[§8.1.3](#813-2026年9月18日の限定公開記録)）
- CWS配信済み`0.1.0`のbytes取得とsourceとの照合（2026年9月18日と9月19日の2回。[§8.1.4](#814-2026年9月18日の配信bytes取得記録)、[§8.1.5](#815-2026年9月19日の初回導線実行と版の再割当)）
- **明示承認に基づく修正版`0.1.1`の審査提出**（2026年9月19日にownerが実行。[§8.1.6](#816-2026年9月19日の011審査提出記録)）
- **`0.1.1`の審査への合格**（2026年9月20日にownerが確認。[§8.1.7](#817-2026年9月20日の011審査通過記録)）
- **明示承認に基づく`0.1.1`の`Unlisted`公開**（2026年9月20日にownerが実行。公開後のvisibility・対象地域・listing URLの読取りと、標準追加できる事前状態の確認まで完了。[§8.1.8](#818-2026年9月20日の011限定公開記録)）
- **CWS配信済み`0.1.1`のbytes取得とsourceとの照合**（2026年9月20日。SHA-256 `d7e48e64…83a170`・8439 byte。[§8.1.9](#819-2026年9月20日の011配信bytes取得記録)）
...
- `TD-11`の`V-12A`からのやり直し。**新しいcampaign IDで行う**
- `TD-11`の`V-12C`で使う`0.1.2`のupload・審査

この分離により、ローカル成果物を完了条件とする`TD-37`は完了です。**`TD-39`は2026年9月18日に完了条件をすべて満たしました**（[§8.1.4](#814-2026年9月18日の配信bytes取得記録)）。**`TD-42`は2026年9月20日に完了条件をすべて満たしました**（[§8.1.9](#819-2026年9月20日の011配信bytes取得記録)）。これにより`TD-11`の保留が解けます。`signed_builds`へ入れる値は`0.1.0`について固定済みですが、`0.1.1`の配信bytes取得、manifest fileの生成、`V-12A`〜`V-12E`の実行が残るため、`TD-11`と`V-12`は未完了です。
...
### 8.1.5. 2026年9月19日の初回導線実行と版の再割当

`TD-11`の`V-12B → V-12D`を初めて実機で実行し、**同意画面のcontent scriptの不具合により`V-12D`の手前で停止しました。** 原因、採らなかった案、再評価条件は[ADR-0005](../../decisions/0005-verify-consent-flow-in-browser-semantics.md)を正とします。

#### 配信bytesの再取得結果

`TD-11`手順3のmanifest入力固定として、配信中の`0.1.0`を1回取得しました。

| 項目 | 結果 |
|---|---|
| SHA-256 | `54142d25…abc43c40`（[§8.1.4](#814-2026年9月18日の配信bytes取得記録)と**一致**） |
| bytes | 8429（同じく一致） |
| 先頭4byte | `Cr24` |
| 署名proof | 3件。うち1件の公開鍵が`crx_id`を導出する |
| sourceとの照合 | `atcoder.js`、`bootstrap.js`、`service_worker.js`、`icon128.png`がbyte単位で一致。`manifest.json`は`update_url`の1行差のみ |

**CWSは`0.1.0`を再packageしていませんでした。** §8.1.4が`TD-11`へ申し送っていた「異同を記録する」点は、異同なしが答えです。

隔離buildの`0.1.0` ZIPも`b0a8d078…4dbbe78`で再現しました。8月26日のupload、9月18日の照合に続く**3つ目の独立したpathでの再現**です。

#### 標準追加で確認できたこと

| 確認 | 結果 |
|---|---|
| developer modeなしの標準追加 | **できた** |
| Google accountの追加・同期 | **不要だった。** 追加を押す前にsign inの勧誘が表示されたが、閉じれば追加できた |
| 表示された権限 | `atcoder.jp`と`127.0.0.1`のCookie・サイトデータの読み取りのみ |
| 固定IDによる導入完了の自動検出 | **できた**（固定ID・版`0.1.0`が一致） |
| Chromeの完全終了と基準templateの一度だけの確定 | **できた**（完全性ID `ab5f4c4d…`） |

限定公開はサインイン不要という[公式docs](https://developer.chrome.com/docs/webstore/cws-dashboard-distribution)の記載どおりでした。サインインでの本人確認が明記されているのは`Private`の配布オプションです。

#### 版の再割当

`0.1.1`は未uploadのまま保持していたため、**修正版を`0.1.1`へ充てます。** 更新testに使う版は`0.1.2`へ繰り下げます。[`prepare.mjs`](../../../scripts/verification/atcoder_v12/prepare.mjs)の版定数も同じ変更で更新しました。
...
[サポートページ](v12-extension-support.md)と[privacy policy](v12-extension-privacy-policy.md)が固定しているreviewer用フィクスチャのSHA-256は、フィクスチャを変更していないため**有効なままです。** ただし`0.1.1`を審査へ出す期間は、この2ファイルへ再び凍結を適用します。

#### 残っている外部操作

**`0.1.1`のuploadと審査提出には、別の明示承認が要ります。** 承認前にCWSの状態を変更しません。手順は段階2（#6〜#9）と同じものを`0.1.1`に対して繰り返します。`0.1.0`は、修正版が配信されるまで現行のcurrent versionとして残します。

### 8.1.6. 2026年9月19日の`0.1.1`審査提出記録

**修正版`0.1.1`を審査へ提出しました。** [ADR-0005](../../decisions/0005-verify-consent-flow-in-browser-semantics.md)の決定に基づく提出です。明示承認を得たうえで、ownerがdashboardで実行しました。
...
| test instructions | 変更なし。username欄・password欄は空欄のまま。フィクスチャを変更していないため記載のSHA-256 `18b3be7b…`は有効 |
...
`0.1.0`では、reviewerが経路2のフィクスチャを起動しても**同意画面から先へ進めなかったはずです。** フィクスチャは`("127.0.0.1", 0)`で動的portへbindし`http://127.0.0.1:<port>/bootstrap`を案内しますが、`0.1.0`の`bootstrap.js`はポートを含む`location.origin`を拒否してcontent scriptを即座に終了していました。両方の挙動を2026年9月19日に実測した帰結です。

**それでも`0.1.0`は審査に合格しました。** `TD-39`の完了条件が「合格はreviewerがフィクスチャを起動した証拠ではない」と書き添えていたとおりの結果です。`0.1.1`で初めて、この経路が最後まで成立する状態になります。

#### 審査中の制約

- [サポートページ](v12-extension-support.md)と[privacy policy](v12-extension-privacy-policy.md)を変更しない。listingの2つのURLがGitHubの`main`を指すため、コミットすると審査担当者が見るページが即座に変わる
- `0.1.0`は、修正版を公開するまでcurrent versionのまま残す
- `0.1.2`（更新test用、`ef64910d…`）は`V-12C`で明示承認を得るまでuploadしない
- **公開には審査提出とは別の明示承認が要る。** 合格の連絡を受けてから30日以内に実施する

審査結果は2026年9月20日に合格を確認しました（[§8.1.7](#817-2026年9月20日の011審査通過記録)）。**この2ファイルの凍結は、`0.1.0`のときと同じく合格をもって解除します。**

### 8.1.7. 2026年9月20日の`0.1.1`審査通過記録

**修正版`0.1.1`は審査に合格しました。** [`TD-42`](../../../TODO.md#td-42-修正版011を公開しv-12の再実行条件を整える)手順1の記録です。
...
| 情報源 | ownerからの報告と、dashboardのステータス画面の表示 |
| 確認日 | 2026年9月20日 |
| dashboardの表示 | 「このドラフトは許可され、2026/10/19 日までに公開できます。」 |
| 公開期限 | **2026年10月19日。** 画面表示のため推定ではない |
| 合格日 | **2026年9月19日ごろ。** 期限表示から30日を逆算した値で、合格日欄そのものは読み取っていない。提出と同日にあたる |
| 対象 | 固定IDのitem、version `0.1.1`、upload ZIP SHA-256 `d283ee36…1eae5ca`（5516 bytes） |
| 公開状態 | **未公開。** deferred publishingを選んだため、合格だけでは公開されない |
| current version | **`0.1.0`のまま。** 公開するまで配信される版は変わらない |
| reviewerからの要望 | 報告なし。経路1（GitHubリリース）の作成要望は発生していない |

不承認ではないため、`TD-42`手順1の停止条件には当たりません。credential共有、Gatekeeper回避、別配布元、手動読込による迂回を検討する場面は発生せず、CWS supportへの問い合わせも発生しません。

**合格だけでは、reviewerが修正後の導線を最後まで動かした証拠になりません。** `0.1.0`も同じ受渡し方式で合格しており、そのときreviewerは同意画面から先へ進めなかったはずだと[§8.1.6](#816-2026年9月19日の011審査提出記録)で実測から述べています。`0.1.1`で経路が成立する状態にはなりましたが、reviewerが実際に起動したかはdashboardから分かりません。`V-12`の前提には使わず、審査が成立したという事実だけを使います。

`0.1.0`は提出の翌日ごろ、`0.1.1`は提出と同日ごろの合格です。**いずれも期限表示からの逆算で、この所要時間を次回の見込みには使いません。**

#### 提出物が`main`から再現できることの再確認

[§8.1.6](#816-2026年9月19日の011審査提出記録)は、提出に使ったbuildの拡張機能source treeが`main`と同一（`8b9f85ec…`）であることを`main`のHEADが`9d7d27d`の時点で確認していました。その後`main`は進んでいるため、合格の記録と同じ日に読み直しました。

| 確認 | 結果 |
|---|---|
| 提出時のcommit `32e05d4`の拡張機能source tree | `8b9f85ec2cf724d6c6f3b192b37e0789dc96b866` |
| `main`のHEAD `1e64e50`の拡張機能source tree | **同一** |

**公開後の配信bytesと照合する基準は、引き続き`main`側のcommitです。** 拡張機能ZIPはsource treeだけから作るため、拡張機能以外のコミットが増えても同じbytesになります。

#### 公開期限

deferred publishingの有効期限は**2026年10月19日**です。2026年8月27日の確認dialogが示した「合格から30日」（[§8.1.1](#811-2026年8月27日の審査提出記録)）と整合します。

**期限まで残り29日です（2026年9月20日時点）。** 期限を過ぎるとdeferred publishingが失効し、再提出が必要になります。再提出は審査待ち時間が再び発生し、`V-12`のやり直し全体が遅れます。

この期限を`TD-11`の実行期間と混同しません。**公開そのものは短時間で終わるため、`V-12`の実行完了を10月19日までに収める必要はありません。** 期限内に行う必要があるのは公開操作だけです。

#### 公開の明示承認で提示する内容

公開は審査提出とは別の外部操作です。**「`TD-42`を実行してよい」「審査は通った」という連絡を、公開の承認へ読み替えません**（[§0](#0-結論)、[作業ガイド §4](../../../CLAUDE.md#4-外部操作には明示承認が必要)）。承認を求めるときは次を提示します。**2026年9月20日、この内容でownerの明示承認を得ました。** 承認記録そのものはリポジトリ外のowner専用領域にあります（[§5](#5-外部操作の承認記録)）。

| 提示項目 | 内容 |
|---|---|
| 操作 | CWS dashboardでdeferred publishingの公開を実行する。1回限り |
| 対象 | 固定IDのitem、version `0.1.1`、upload ZIP SHA-256 `d283ee36…1eae5ca`（5516 bytes） |
| `0.1.0`からの変更 | `bootstrap.js`のloopback判定1行のみ。権限、host権限、content script対象、Privacy、データ使用、Store listing、Distributionの申告は変更していない |
| 公開範囲 | 料金なし・`Unlisted`・日本のみ。**listing URLを知る日本の利用者が、通常のChromeで標準追加できる状態になる。** 検索結果には出ないが、指定accountだけに制限する`Private`ではない |
| 配信される版の置き換え | **current versionが`0.1.0`から`0.1.1`へ変わる。** 2026年9月19日に`0.1.0`を追加したprofileが残っている場合、Chromeの自動更新で置き換わりうる（**未実測**）。[ADR-0005](../../decisions/0005-verify-consent-flow-in-browser-semantics.md)によりcampaign `v12-2026-09-19-01`は無効で、`TD-11`のやり直しは新しいcampaign IDで`V-12A`から行うため、この置き換えで失われる有効な検証結果はない |
| 費用 | 発生しない |
| 期限 | **2026年10月19日**（dashboard表示）。超過するとdeferred publishingが失効し、再提出が必要になる |
| 停止方法 | 承認済みownerがdashboardからunpublishする。追加済みの複製、Keychain項目、AtCoder側sessionは別に扱う（[§6](#6-停止とcampaign終了)） |
| 公開後に行うこと | developer modeなしに標準追加できる事前状態の確認と、配信bytesの取得・照合だけ。`TD-11`の基準templateは作らない |
| 記録先 | リポジトリ外のowner専用領域の承認記録（[§5](#5-外部操作の承認記録)） |

#### 実行者と手順（2026年9月20日に承認を取得。実行待ち）

**公開操作はownerが行います。** publisher用Google accountの認証情報はリポジトリにも作業環境にも置かないため（[作業ガイド §5](../../../CLAUDE.md#5-リポジトリへ書かないもの)）、AIはdashboardへ到達できません。AIが担うのは、実行前の提示、実行後の記録、`TD-11`への引き渡しです。

**承認は公開操作1回だけを対象とします。** 有効期限は公開期限と同じ2026年10月19日で、期限を過ぎたら承認は消費されずに失効し、取り直します。
...
|---|---|
| item | 固定IDのitem。ほかのitemではない |
| version | `0.1.1`。`0.1.0`でも`0.1.2`でもない |
| 状態 | 「このドラフトは許可され、2026/10/19 日までに公開できます。」 |
| Distribution | 料金なし・`Unlisted`・日本のみ |
| 費用 | 表示されない |
...
| 実行時刻（UTC） | 承認記録の`approved at`と実行記録 |
| 公開後のitem状態 | 「公開済み」等の表示。反映待ちの有無 |
| current version | **`0.1.1`へ置き換わったこと** |
| visibility | `Unlisted`のまま変わっていないこと |
| 対象地域 | 日本のみのまま変わっていないこと |
| listing URL | `0.1.0`のときと同じURLかどうか。`TD-11`の最終campaign manifestで使う |

その後、通常のChromeでlisting URLを開き、developer modeなしに標準追加できる事前状態であることを確認します。続けて配信中の`0.1.1`のbytesを取得し、[§8.1.4](#814-2026年9月18日の配信bytes取得記録)と同じ方法でリポジトリのsourceと1ファイルずつ照合します。**取得できなければ`TD-11`へ進みません。**

#### 審査中の凍結の解除

[サポートページ](v12-extension-support.md)と[privacy policy](v12-extension-privacy-policy.md)へ`0.1.1`の審査期間中に再適用していた凍結は、**合格をもって解除します。** `0.1.0`のとき（[§8.1.1](#811-2026年8月27日の審査提出記録)）と同じ扱いです。解除後も次を守ります。

- 配信中の版の手順・開示内容と食い違う編集を行わない。この2ページは公開後も利用者が見るページである
- 次に審査へ出す版（`0.1.2`）の審査期間には、同じ凍結を再適用する

**`0.1.1`を公開して配信される版が変わっても、この2ファイルを直す必要はありません。** 2026年9月20日に読み直したところ、版番号を直接書いている箇所がありません。フィクスチャのSHA-256 `18b3be7b…`も、フィクスチャを変更していないため有効なままです。

### 8.1.8. 2026年9月20日の`0.1.1`限定公開記録

**修正版`0.1.1`を限定公開しました。** `TD-42`手順3の記録です。dashboard操作はownerが実行し、AIは提示と記録だけを行いました。

| 項目 | 確認結果 |
|---|---|
| 実行者 | owner |
| 実行日 | 2026年9月20日。**分単位の時刻は未記録。** `0.1.0`のとき（[§8.1.3](#813-2026年9月18日の限定公開記録)）と同じく、dashboardに実行時刻の表示がなく、記録の目的には日付で足りる |
| 実行前の照合 | [§8.1.7](#817-2026年9月20日の011審査通過記録)の5項目（item、version、状態、Distribution、費用）が**すべて一致した**とownerが報告した |
| item状態 | 「公開済み」 |
| current version | **`0.1.1`。** `0.1.0`から置き換わった |
| 費用 | 発生しない |
| 公開期限 | **消化した。** 2026年10月19日の期限はこの公開で用済みになった |

**配信される版は`0.1.1`になりました。** `0.1.0`はcurrent versionではなくなりました。[サポートページ](v12-extension-support.md)と[privacy policy](v12-extension-privacy-policy.md)は版番号を直接書いていないため、記述を変える必要はありません（[§8.1.7](#817-2026年9月20日の011審査通過記録)）。

#### 公開後の読取り

`0.1.0`のとき（[§8.1.3](#813-2026年9月18日の限定公開記録)）と同じ3件を、公開後に読み取りました。**情報源はownerの報告です。**

| 読み取った項目 | 結果 |
|---|---|
| visibility | **`Unlisted`のまま。** 変わっていない |
| 対象地域 | **日本のみのまま。** 変わっていない |
| listing URL | **`0.1.0`のときと同じ。** 値はowner専用領域にあり、リポジトリへは書かない |

3件とも承認内容と一致するため、[§6](#6-停止とcampaign終了)の停止手順を検討する場面は発生しません。**値そのものはowner専用領域へ記録し、リポジトリへは書きません**（[作業ガイド §5](../../../CLAUDE.md#5-リポジトリへ書かないもの)）。listing URLについて本書へ書くのは、`0.1.0`のときと同じかどうかだけです。

listing URLが変わっていないため、`TD-11`の最終campaign manifestの`extension.listing_url`と`extension.id`は、[§8.1.4](#814-2026年9月18日の配信bytes取得記録)の引渡し表の値をそのまま`0.1.1`へ引き継げます。**版に依存して変わるのは`target_version`と`signed_builds`だけです。**

#### 標準追加できる事前状態の確認

**確認できました。** `TD-42`手順4の記録で、情報源はownerの報告です。`0.1.0`のとき（[§8.1.3](#813-2026年9月18日の限定公開記録)）と同じ方法で確認しました。

| 確認項目 | 結果 |
|---|---|
| profile | publisher accountにsign inしていない通常modeのprofile。シークレットモードとゲストは使わない |
| 「Chromeに追加」ボタン | **活性で押せる状態** |
| developer modeの要求・警告 | **なし** |
| 表示されたversion | **`0.1.1`** |
| ボタンの押下 | **押していない。** 標準追加そのものは`TD-11`の`V-12B`で行う |

**`Unlisted`で公開した`0.1.1`が、URLを知る日本の利用者に対して、developer modeも手動読込も要求せずChromeの標準画面から追加できる状態にある**ことを意味します。`0.1.0`で確認した事前状態が、修正版でも成り立っています。

`0.1.0`のときに記録した制約はそのまま当てはまります。シークレットモードとゲストでは追加操作そのものが禁止されているため、追加できない表示を見ても**itemの状態が原因かモードが原因かを区別できません**（[§8.1.3](#813-2026年9月18日の限定公開記録)）。確認に使った準備確認profileは破棄します。

**`TD-11`の基準templateはまだ作りません。** 手順4の範囲外です。

残る手順5（配信bytesの取得と照合）は同日に実施しました（[§8.1.9](#819-2026年9月20日の011配信bytes取得記録)）。

### 8.1.9. 2026年9月20日の`0.1.1`配信bytes取得記録

**CWSが配信する`0.1.1`のbytesを取得し、リポジトリのsourceと照合できました。** `TD-42`手順5の記録です。取得はownerが行い、展開と照合はリポジトリ外の一時pathで行いました。**固定IDと取得URLはリポジトリへ書きません。**

| 項目 | 結果 |
|---|---|
| 方法 | Chromeが更新に使うendpoint `clients2.google.com/service/update2/crx`へ、固定IDと`prodversion`を付けて1回requestした。`0.1.0`（[§8.1.4](#814-2026年9月18日の配信bytes取得記録)）と同じ方法 |
| `prodversion` | `152.0.7977.84` |
| 形式 | 先頭4byteが`Cr24`。CRX3で、headerが1308 byte、ZIP部が7119 byte |
| 署名proof | 3件（RSA 2件、ECDSA 1件）。`0.1.0`の観測（[§8.1.5](#815-2026年9月19日の初回導線実行と版の再割当)）と同じ件数 |
| **SHA-256** | **`d7e48e642eb5a9492b871b6f37448d7d432a8c3c8e4ffc742facbbf36083a170`** |
| **bytes** | **8439** |
| 収録内容 | `manifest.json`、`atcoder.js`、`bootstrap.js`、`service_worker.js`、`icon128.png`、`_metadata/verified_contents.json` |

このSHA-256は、uploadした`0.1.1` ZIPの`d283ee36…1eae5ca`とも、それまで配信されていた`0.1.0`の`54142d25…abc43c40`（8429 byte）とも**一致しません。** CWSが版ごとに再署名するためで、`0.1.0`のときと同じ理由です。
...
| file | 結果 |
|---|---|
| `atcoder.js` | **byte単位で一致**（2486 byte） |
| `bootstrap.js` | **byte単位で一致**（1716 byte） |
| `service_worker.js` | **byte単位で一致**（6150 byte） |
| `icon128.png` | **byte単位で一致**（939 byte） |
| `manifest.json` | CWSが`"update_url": "https://clients2.google.com/service/update2/crx"`の1行を挿入した差分のみ（784 byte → 850 byte） |
| `_metadata/verified_contents.json` | **CWS側で追加されたもの**（1747 byte）。署名検証用のmetadataで、sourceには存在しない |

再buildした`0.1.1` ZIPのSHA-256は`d283ee36850baa14cf313d5228ecc11908a86aac7f7b4e380741a28621eae5ca`・5516 byteで、2026年9月19日にuploadした値と一致しました。**`0.1.1`としては、提出用build、`main`からの再build（[§8.1.6](#816-2026年9月19日の011審査提出記録)）に続く3つ目の独立したpathでの再現です。**

**結論は`0.1.0`と同じです。** CWSが利用者へ配信するのは、リポジトリのsourceから再現できるbytesに、CWSが付ける`update_url`行と署名metadataを加えたものだけで、第三者のコードや追加の権限は入っていません。

#### 修正が配信物に入っていることの確認

[ADR-0005](../../decisions/0005-verify-consent-flow-in-browser-semantics.md)の修正が、**CWSが実際に配信するbytesの中にある**ことを確認しました。`0.1.1`を出した目的そのものであるため、sourceとの一致とは別に確認しています。

| 確認 | 結果 |
|---|---|
| 配信された`bootstrap.js`のloopback判定 | `location.protocol`・`location.hostname`・`location.pathname`で判定している |
| `0.1.0`にあった`location.origin !== "http://127.0.0.1"` | **0件。** 配信物に残っていない |

動的な待受番号が付いたURLでもcontent scriptが即座に`return`しない版が、配信される状態になりました。**ただし、実browserで同意画面から先へ進めることはまだ観測していません。** 確認したのは配信物の中身であり、`V-12D`の成立は`TD-11`のやり直しで判定します。[ADR-0005](../../decisions/0005-verify-consent-flow-in-browser-semantics.md)が問題にしたのは「ソースの文字列検査を成立証拠にしたこと」なので、**ここでの文字列の確認も成立証拠にしません。**

#### 権限の確認

配信された`manifest.json`を読み、`0.1.0`から権限が増えていないことを確認しました。

| 項目 | 配信物の値 |
|---|---|
| `version` | `0.1.1` |
| `permissions` | `cookies`、`storage` |
| `host_permissions` | `https://atcoder.jp/*`、`http://127.0.0.1/*` |
| content scriptの対象 | `http://127.0.0.1/*`、`https://atcoder.jp/settings*` |

`debugger`、`webRequest`、`tabs`、`scripting`、`nativeMessaging`の要求はありません。申告・審査した範囲と一致します。

#### 取得方法の位置づけ

[§8.1.4](#814-2026年9月18日の配信bytes取得記録)に書いたとおり、**この取得方法は公式docsに書式の記載がありません。** 2026年9月18日に続いて9月20日も成功したという事実の記録であり、将来も同じ方法で取得できる保証はありません。2回成功したことを一般規則として扱いません。

**CWSが同じversionを再packageすればhashが変わる可能性があります。** `TD-11`の`V-12A`では本記録との一致を検査するのではなく、そのとき配信されているbytesを取り直してcampaign manifestへ固定し、本記録との異同を記録します。

#### `TD-11`へ引き渡す値

[§8.1.4](#814-2026年9月18日の配信bytes取得記録)の引渡し表を`0.1.1`で置き換えます。`extension.id`と`extension.listing_url`は`0.1.0`のときから変わっていないため、owner専用領域の値をそのまま使います（[§8.1.8](#818-2026年9月20日の011限定公開記録)）。

| manifestのfield | 引き渡す値 | 置き場所 |
|---|---|---|
| `extension.id` | 32文字の固定ID。`0.1.0`から変更なし | owner専用領域 |
| `extension.listing_url` | 限定公開のlisting URL。`0.1.0`から変更なし | owner専用領域 |
| `extension.target_version` | **`0.1.1`** | 本書 |
| `extension.signed_builds[].sha256` | **`d7e48e64…83a170`** | 本書 |
| `extension.signed_builds[].bytes` | **`8439`** | 本書 |
| local buildの`0.1.1` ZIP | `d283ee36…1eae5ca`・5516 byte | 本書・`build-index.json` |
| helper、protocol、同意版、template schema | `TD-37`の`build-index.json`と`campaign-manifest.example.json`の形式 | リポジトリ |
| Chrome・OSの版 | **実行時に確定** | `TD-11` |

**`TD-42`の完了条件はこれですべて満たしました。** `TD-11`は[ADR-0005](../../decisions/0005-verify-consent-flow-in-browser-semantics.md)の定めにより`TD-42`の完了まで保留でしたので、保留が解けます。やり直しは新しいcampaign IDで`V-12A`から行います。
...
**`0.1.0`については、段階1から段階3の#11まで完了しました。** #12は「`0.1.1`を`TD-11`まで上げない」でしたが、[ADR-0005](../../decisions/0005-verify-consent-flow-in-browser-semantics.md)で`0.1.1`を修正版へ充てたため、更新testに使う版は`0.1.2`に読み替えます。

**修正版`0.1.1`は、段階2の#7〜#9と同じ手順を繰り返して合格まで到達しました**（[§8.1.6](#816-2026年9月19日の011審査提出記録)、[§8.1.7](#817-2026年9月20日の011審査通過記録)）。段階3の#10（公開）も2026年9月20日に明示承認を得て実行し（[§8.1.8](#818-2026年9月20日の011限定公開記録)）、#11（配信bytesの取得と照合）も同日に完了しました（[§8.1.9](#819-2026年9月20日の011配信bytes取得記録)）。**`0.1.1`については段階1から段階3までが揃い、`TD-42`は完了です。** 残るのは`TD-11`の`V-12C`で使う`0.1.2`のuploadだけで、これには別の明示承認が要ります。段階1に外部影響はありません。
