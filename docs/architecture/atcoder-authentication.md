...
「AlgoLoomだけが読める」とは表示しません。macOS Keychainの項目のアクセス制御は呼び出し元の実行ファイルへ紐づきますが、§3.6の契約により認証ヘルパーはPythonの実行環境の上で動作するため、信頼される実行ファイルはインタプリタになります。専用の実行ファイルを用意しても、ad-hoc署名では再ビルドのたびに識別子が変わり、更新のたびにアクセス制御が失効します。3つのOSでの実際の保証範囲と表示文言は`TD-12`で確認します。macOSの観測結果は§4.1.1にあります。**WindowsとLinuxは未観測のため、表示文言は確定していません。** Windowsの観測は`TD-47`、Linuxの観測は`TD-48`、3つを踏まえた表示文言の確定は`TD-46`で行います。
...
| `M-17` | 統合（§4.1.1で観測済み） | 統合（未観測） | 統合（未観測） | 保証範囲そのものがOS差。観測して表示文言を決める |
...
- **WindowsとLinuxの`M-17`の実観測。** macOSだけの観測から3つのOSの表示文言を決めません
- **CPU architectureと通常Google Chromeの対応版の固定**（`TD-31`）

判断の経緯は[ADR-0010](../decisions/0010-build-the-three-os-auth-matrix-before-v12-passes.md)にあります。Windowsの観測は`TD-47`、Linuxの観測は`TD-48`、確定は`TD-46`で扱います。
