...
>
> 状態: 認証方式の正本。方式Cは技術検証用。方式Aは現行CDP候補を不適合とし、通常Chrome状態を維持するCookie限定取得境界を`p0-23`で実証済み。`p1-02`で期限を推測しない認証失敗分類、`p1-03`でCookie更新・明示期限・再起動・失効・競合・秘密情報保管庫障害の安全停止を実証済み。`TD-09`では署名済みChrome拡張機能と認証付き折返し通信の組み合わせを次の実機検証候補に選定し、`TD-10`で追加P0 `V-12`の検証計画を確定した。製品形態の成立は未検証であり、`V-12`の合格前に採用確定とは扱わない
>
> 決定日: 2026年8月11日
>
> 最終更新日: 2026年8月25日
