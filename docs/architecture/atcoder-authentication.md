...
3. `TD-09`の机上比較では、**公式拡張機能配布基盤で限定公開する署名済みChrome拡張機能と、認証付き折返し通信を組み合わせる形態**を`V-12`へ進める一案に選定しました。これは実機検証対象の選定であり、公式拡張機能配布基盤の審査通過、標準画面からの追加、実サービスとの互換性、配布物の安全性を`V-12`で確認するまで製品採用を確定しません。**`V-12`は2026年9月21日に合格しました**（[`v12-05`](../verification/judge-adapter/results/2026-09-21-v12-05.md)）。成立した範囲は[§3.1.6](#316-v-12の実機検証で成立した範囲)にあります。**合格は「成立を観測した」ことであり、製品採用の確定とは別です。** 確定は利用者へ示す約束と外部作用の範囲に関わるため、人の判断を経て行います。
...
#### 3.1.6. `V-12`の実機検証で成立した範囲

2026年9月21日のcampaign `v12-2026-09-21-02`で、`V-12A`〜`V-12E`が同じcampaign manifestで合格しました（[`v12-05`](../verification/judge-adapter/results/2026-09-21-v12-05.md)）。**成立したのは次の範囲です。**

| 範囲 | 観測した事実 |
|---|---|
| 初回セットアップ | 開発者向け設定なしで、公開済みlistingから標準追加できた。固定IDで導入完了を自動検出し、Chrome完全終了後に基準templateを一度だけ確定した。ChromeへのGoogleアカウント追加も同期も求めていない |
| 同意と限定取得 | 同意版1.0を表示したうえで、`REVEL_SESSION`を厳密に1件だけ取得した。他のCookieを読んでいない |
| 本人照合と保存 | 本人照合が一致した値だけをOSの秘密情報保管庫へ保存し、**新しいprocessから同じ本人を再照合できた。** `GET /settings`は上限と同数の2回 |
| `submit`中の再認証 | 保存済みlocal sessionが無いことを検出し、理由と中止方法を示してからbrowserを自動起動した。**追加の認証commandとYes/No確認は0件**で、同じbrowserで提出確認画面まで進んだ |
| 提出pageへの受け渡し | 提出確認画面の操作から、**同じtabがAtCoderの提出pageへ着いた。** 手動のページ探索とcopy-and-pasteを求めていない |
| 安全側停止と後始末 | 版・権限・配布元の不一致、templateの破損、秘密情報保管庫の障害、profile lockでいずれも安全側停止した。実行用profile、待受、process、一時資源の残存は0件 |

**成立していない範囲、または確かめていない範囲です。**

- **提出そのものは行っていません。** `V-12E`は提出pageへ着くところまでで、提出formの操作は0件です
- **代表環境はmacOS 26.5・arm64の1つだけです。** 3つのOSでの成立は[`TD-12`](../../TODO.md#td-12-3つのosの認証検証マトリクスを作る)が扱います
- **拡張機能の更新導線（`0.1.2`）は必須caseに含みません**（[ADR-0009](../decisions/0009-required-cases-for-v12c.md)）
- 提出ページでcontent scriptとTurnstileが共存するかは、[`TD-40`](../../TODO.md#td-40-提出ページのcontent-scriptとturnstileの共存を検証する)の範囲です

**この合格が覆る条件は結果無効化規則のとおりです。** 拡張機能の固定ID・版・配布元・権限・source、helper、protocol、同意版が変われば、`V-12A`〜`V-12E`をやり直します。

### 3.2. 利用者の導線

次のUX契約を採用します。実現手段であるテンプレート複製等は、**代表環境では`V-12`の合格をもって実機で成立しました**（[§3.1.6](#316-v-12の実機検証で成立した範囲)）。3つのOSでの成立は`TD-12`が扱います。詳細な画面遷移は[AtCoder認証UX設計](../project/atcoder-authentication-manual-operation-automation.md)を参照します。
...
「AlgoLoomだけが読める」とは表示しません。macOS Keychainの項目のアクセス制御は呼び出し元の実行ファイルへ紐づきますが、§3.6の契約により認証ヘルパーはPythonの実行環境の上で動作するため、信頼される実行ファイルはインタプリタになります。専用の実行ファイルを用意しても、ad-hoc署名では再ビルドのたびに識別子が変わり、更新のたびにアクセス制御が失効します。3つのOSでの実際の保証範囲と表示文言は`TD-12`で確認します。macOSの観測結果は§4.1.1、Windowsの観測結果は§4.1.2にあります。**Linuxが未観測のため、表示文言は確定していません。** Linuxの観測は`TD-48`、3つを踏まえた表示文言の確定は`TD-46`で行います。

#### 現在の設計で守れること・守れないこと

上の表を、脅威の側から並べ直すと次のようになります。**「守れない」を減らすために対策を足すのではなく、守れない範囲を利用者へ正確に伝えることを選んでいます。**

| # | 脅威 | 現在の設計 | 理由 |
|---:|---|---|---|
| 1 | 端末を紛失し、ディスクを別のOSから読まれる | **守れる** | 保存時に暗号化され、鍵はログインに紐づく |
| 2 | バックアップまたはCloud同期へ平文で含まれる | **守れる** | 保管庫へ入れる。保管庫を使えない場合も平文へ切り替えず、提出だけを止める |
| 3 | 履歴DB、作業領域、通常ログ、成果物へ残る | **守れる** | `argv`、環境変数、設定ファイル、通常ログへ渡さない |
| 4 | 別の利用者アカウントから読まれる | **守れる** | 保管庫が利用者ごとに分離する |
| 5 | ロック中に読まれる | **守れる** | OSのロック解除状態に従う |
| 6 | **同じ利用者として動作する他のプロセスから読まれる** | **守れない** | OSの隔離の単位が利用者アカウントであるため。§4.1.1・§4.1.2で実測した |
| 7 | 端末を管理者権限で操作できる者 | **守れない** | 保管庫の外側から回避できる |
| 8 | 未確認の値を保存してしまう | **守れる** | 本人照合に成功した値だけを確定保存する（§3.5） |
| 9 | ローカルの削除をAtCoder側の失効と取り違える | **守れる（文言で区別する）** | ローカルの削除はAtCoder側の失効を意味しない |

**6を自前の暗号化で解決できません。** 鍵を保管庫へ置けば同じ境界へ戻り、配布物へ埋め込めば秘密になりません。利用者のpassphraseやハードウェアへ置いても、**同じ利用者として動作できる者はAlgoLoom自身を書き換えられる**ため、復号後の値を取り出せます。仕組みの説明は[OSの秘密情報保管庫と信頼境界](../research/os-secret-store-and-trust-boundary.md)にあります。

### 4.1.1. macOSで観測した実際の保証範囲

> 観測日: 2026年9月20日。macOS 26.5、arm64、login keychain、Apple署名のPython 3.9。`TD-12`の一部として実施

[未決事項 7.3](../project/unresolved-decisions.md#73-秘密情報保管庫が保証する範囲と表示文言)が求める2点を、製品が使うものと同じAPI（`SecItemAdd`と`SecItemCopyMatching`）で観測しました。検証用の項目は観測後に削除しています。次で再現できます。
...
| 観測したこと | 結果 |
|---|---|
| 読み出しを許可される実行ファイル | 作成したプロセスの実行ファイル1件だけがアクセス制御へ記録される。Pythonから作成した場合に記録されるのは、**AlgoLoomのコードではなくインタプリタ**（`Python.app`） |
| 同じ利用者として動作する他のプロセスから読めるか | **読める。** 同じインタプリタを起動できる任意のプロセスが、確認画面なしで値を取得できた。別のスクリプトファイル、別の作業ディレクトリ、仮想環境経由のいずれでも同じ |
| AlgoLoomの更新時に再認可を求められるか | **求められない。** 信頼される実行ファイルがインタプリタであるため、AlgoLoom側のコードが変わってもアクセス制御は変わらない |
| 専用の実行ファイルにした場合 | ad-hoc署名の実行ファイルが作成した項目は、**同じソースを再ビルドしただけで読めなくなった。** コード署名の識別子が変わるため、更新のたびに再認可が要る |
| 別のプログラムから読んだ場合 | `/usr/bin/security`からの読み出しでは利用者への確認画面が出た。承認すれば読める |
| 削除 | 読み出しの認可なしで削除できた。「削除対象と削除方法が一意に定まること」の保証と矛盾しない |

**アクセス制御の信頼実行ファイル一覧は、汎用インタプリタを締め出しませんでした。** ad-hoc署名の実行ファイルが作成し、読み出し許可にその実行ファイル1件だけを持つ項目を、Apple署名のPythonが確認画面なしで、かつ対話を禁止した状態でも読み出せています。同じ条件で`/usr/bin/security`と再ビルド後のad-hoc実行ファイルは読めなかったため、呼び出し元のコード署名の種類が影響していると考えられますが、**仕組みは特定していません。** 観測した事実としてだけ記録し、配布形態が決まった時点で再確認します。

この観測は上の表を変えません。「同じ利用者として動作する他のプロセスから読めないこと」を保証しない、という記述を裏付けます。§3.6の契約どおりPythonの実行環境の上で動作させる限り、**専用の実行ファイルを用意しても保護は強くならず、再ビルドのたびに再認可を求める副作用だけが増えます。**
...
| 観測したこと | 結果 |
|---|---|
| 読み出しを許可される実行ファイル | **記録されない。** Credential Managerの`CRED_TYPE_GENERIC`の項目は利用者のアカウントへ紐づき、作成した実行ファイルをアクセス制御として保持しない |
| 同じ利用者として動作する他のプロセスから読めるか | **読める。確認画面は出ない。** 別の実行ファイルである`powershell.exe`が、対話を禁止した状態で値をそのまま取得できた |
| AlgoLoomの更新時に再認可を求められるか | **求められない。** 実行ファイルへの紐付けがないため、AlgoLoom側が何に変わっても読み出しは変わらない |
| 専用の実行ファイルにした場合 | **macOSと違い、選択肢にならない。** 紐付ける先がないため、専用の実行ファイルを用意しても保護は変わらない |
| 削除 | 読み出しの認可なしで削除できた。「削除対象と削除方法が一意に定まること」の保証と矛盾しない |

**macOSとの違いは、保護の強さではなく仕組みです。** macOSは実行ファイル単位のアクセス制御を持ちながら、§4.1.1のとおり汎用インタプリタを締め出しませんでした。Windowsにはアプリ単位の紐付けがそもそもありません。**出発点が違うにもかかわらず、「同じ利用者として動作する他のプロセスから読める」という結論は両方で同じです。**

この観測は§4.1の表を変えません。macOSと同じく、「同じ利用者として動作する他のプロセスから読めないこと」を保証しない、という記述を裏付けます。**表示文言はここで確定しません。** Linuxの観測が`TD-48`で揃ってから`TD-46`で決めます。

#### 4.1.2.1. 最初の実行が反対の結果を報告したこと

**1回目の実行は`other-executable`を「読めなかった」と報告しました。これは観測物の不具合であり、Windowsの保護ではありません。** 記録から消さずに残します。

観測物がPowerShellへ渡していた`CREDENTIAL`構造体の読み出し位置が、32 bitのポインタ幅を前提にしていました。64 bitでは`CredentialBlobSize`が32、`CredentialBlob`が40の位置にあります。誤った位置からは長さ0が読め、値は空文字になります。**`CredReadW`は成功しているのに取り出した値が空になるため、「読めなかった」と区別が付きませんでした。**

直したのは次の2点です。

- 読み出し位置をポインタ幅から決める。64 bitと32 bitのどちらでも正しい位置を使う
- **読み出しに成功して値が空だった場合を、別の結果として出す。** 黙って「読めなかった」に落とさない。観測物は`判定できなかった`という結果値でこれを表す

2点目は、同じ取り違えが別のOSや別のポインタ幅で再び起きたときに、結果ではなく不具合として見えるようにするためです。**保証範囲の観測では、「読めなかった」を誤って報告することが最も危険です。** 実際には読めるものを読めないと記録すれば、その上に過大な表示文言を置いてしまいます。
...
> 状態: **暫定。** `V-12`の合格前に確定としません。macOS以外の秘密情報保管庫は未観測です（§6.2.8）

§6が求める3つのOSの検証マトリクスを、環境と確認項目の組に分け、それぞれを§6.1のどの検証層で確認するかを定めます。

#### 6.2.1. 行にする対象環境

正式対象のhost OSは[可搬性設計 §3.2](language-and-platform-portability.md#32-host-os)が定めます。CPU architectureと通常Google Chromeの正確な対応版は`TD-31`が固定するため、ここでは固定前の暫定として環境IDを置きます。

| 環境ID | host OS | CPU architecture | 秘密情報保管庫 | 最終確認に使う実体 |
|---|---|---|---|---|
| `ENV-MAC-ARM64` | native macOS | arm64 | macOS Keychain | 物理端末 |
| `ENV-MAC-X64` | native macOS | x86_64 | macOS Keychain | 未確定（`TD-18`。§6.2.7） |
| `ENV-WIN-X64` | native Windows | x86_64 | WindowsのOS保護領域 | 物理端末 |
| `ENV-WIN-ARM64` | native Windows | arm64 | WindowsのOS保護領域 | 未確定（§6.2.7） |
| `ENV-LIN-X64-SS` | native Linux | x86_64 | Secret Service（デスクトップセッションあり） | 未確定（`TD-18`） |
| `ENV-LIN-X64-NOSS` | native Linux | x86_64 | Secret Serviceを利用できない | 未確定（`TD-18`） |
| `ENV-LIN-ARM64` | native Linux | arm64 | Secret Service | 未確定（§6.2.7） |

通常Google Chromeの版は固定値にせず、**更新前と更新後の2点**として扱います。更新そのものが確認対象（`M-09`）であるためです。

#### 6.2.2. 列にする確認項目

§6の完了条件と§6.1の境界を、確認できる単位へ分けます。「対応」列は、この項目が満たすMVP完了条件と[`F-SUBMIT-01`の要件](../../spec/features.md#81-atcoder認証状態確認f-submit-01)、受け入れシナリオを指します。

| ID | 確認項目 | 対応 |
|---|---|---|
| `M-01` | 対応するChrome本体を一意に検出する | [MVP §5.1](../product/mvp.md#51-主要導線)、`R7` |
| `M-02` | 専用の`--user-data-dir`で可視起動する | MVP §5.1、`E2E-30` |
| `M-03` | 利用者による中断、タイムアウト、ブラウザ異常終了を通常の中断として扱う | [MVP §5.2](../product/mvp.md#52-障害と回復)、`R9` |
| `M-04` | プロセスツリーとファイルロックを含めて回収し、孤児プロセスと一時プロファイルを残さない | MVP §5.2、`R9`、`E2E-30` |
| `M-05` | 署名済み限定公開拡張機能を標準追加画面から追加する | [MVP §5.3](../product/mvp.md#53-利用者体験)、`R10` |
| `M-06` | 固定ID、バージョン、配布元、権限セットを確認する | `R10` |
| `M-07` | 導入完了を自動検出する | `R10` |
| `M-08` | テンプレートプロファイルを一度だけ確定する | `R10` |
| `M-09` | Chromeと拡張機能の更新前後で複製と完全性を確認する | MVP §5.1 |
| `M-10` | 実行用の一時プロファイルを起動し破棄する | `E2E-30` |
| `M-11` | 完全性が成立しない場合に再設定する | MVP §5.2 |
| `M-12` | 既存プロファイルを参照していない | MVP §5.1、`R8`、`E2E-30` |
| `M-13` | 認証付き折返し通信を確立する | `R7` |
| `M-14` | 認証ヘルパーの起動、取消、異常終了、バージョン不一致で安全側停止する | MVP §5.2、`R9` |
| `M-15` | 秘密情報保管庫へ保存、取得、更新、削除する | MVP §5.1、`R8` |
| `M-16` | 秘密情報保管庫を利用できない場合に平文へ退避せず提出だけを停止する | MVP §5.2 |
| `M-17` | 秘密情報保管庫が実際に保証する範囲を観測し、表示文言を過大にしない | MVP §5.3 |
| `M-18` | ログ、エラー、履歴DB、エクスポート、配布物へ秘密情報を混入させない | MVP §5.1、`R8` |
| `M-19` | アカウント変更、期限切れ、ページ構造の変更で送信前に停止する | MVP §5.2、`R6` |
| `M-20` | 初回、再認証、`submit`からの再認証を一往復で完了する | MVP §5.3、`R7`、`R10` |
| `M-21` | 認証後に同じブラウザで提出確認画面へ到達する | MVP §5.1 |

`TD-15`が認証UXの受け入れシナリオを追加した場合は、追加分をこの表へ行として足します。件数を固定しません。

#### 6.2.3. 検証マトリクス

列を3つのOSにまとめます。同じOSの環境IDどうしで確認層が変わるのは`M-01`と`M-05`〜`M-09`、`M-15`だけで、その差は§6.2.4で上書きします。21項目を7環境へ展開すると同じ内容の写しが増え、片側だけ直したときに食い違うためです。

凡例: **契約**＝OS非依存の契約テスト / **統合**＝3 OS統合テスト / **配布**＝配布導線テスト / **smoke**＝実サービスsmoke / **手動**＝人が行う受け入れ操作（無人化しない）

| ID | native macOS | native Linux | native Windows | OS差として確認すること |
|---|---|---|---|---|
| `M-01` | 契約＋統合 | 契約＋統合 | 契約＋統合 | `.app` bundle / ディストリビューションとパッケージ名 / `.exe`の配置と起動規則 |
| `M-02` | 統合 | 統合 | 統合 | 可視デスクトップセッションの有無 |
| `M-03` | 契約＋統合 | 契約＋統合 | 契約＋統合 | ウィンドウ終了と異常終了の通知経路 |
| `M-04` | 統合 | 統合 | 統合 | バックグラウンドプロセスと使用中ファイル / プロセスグループとデスクトップ環境 / プロセスツリーと使用中ファイルの削除制約 |
| `M-05` | 配布＋手動 | 配布＋手動 | 配布＋手動 | 通常Google Chrome（Chromiumではない）が対象OSに導入できること |
| `M-06` | 契約＋配布 | 契約＋配布 | 契約＋配布 | なし。固定ID、権限セット、配布元は3つのOSで共通の契約（§3.4） |
| `M-07` | 配布 | 配布 | 配布 | 拡張機能の導入状態を読む位置 |
| `M-08` | 統合 | 統合 | 統合 | プロファイルとキャッシュの配置（§3.4） |
| `M-09` | 配布 | 配布 | 配布 | Chromeの更新経路が違う。自動更新 / パッケージ管理 / 自動更新のどれで更新が届くかと、検証中に更新を止められるか |
| `M-10` | 統合 | 統合 | 統合 | 一時プロファイルの配置とファイルロック |
| `M-11` | 契約＋統合 | 契約＋統合 | 契約＋統合 | なし |
| `M-12` | 契約＋統合 | 契約＋統合 | 契約＋統合 | 既存プロファイルの配置。触れていないことを観測する対象になる |
| `M-13` | 契約 | 契約 | 契約 | ループバックの待受制限とファイアウォールの確認 |
...
| `M-21` | smoke＋手動 | smoke＋手動 | smoke＋手動 | なし |
...
無人化しない操作は次の4つだけです。これ以外を「手動」で済ませず、確認層と自動テストを割り当てます。

| 手動操作 | 現れる確認項目 | 無人化しない理由 |
|---|---|---|
| 署名済み拡張機能の追加と権限承認 | `M-05` | Chromeの利用者承認とAlgoLoomの同意境界であり、検証環境の忠実度を上げても無人化しない（§6.1） |
| AtCoderへのログイン | `M-20`、`M-21` | 利用者本人の資格情報を扱い、AlgoLoomへ入力させない（§8） |
| Turnstileの操作 | `M-20`、`M-21` | 自動突破と回避を行わない（§8） |
| 最後の提出ボタン | ― | `V-12E`でも行わず、MVPでも自動化しない（§3.3） |

自動化する製品処理は手動にしません。`M-07`の導入検出、`M-08`のテンプレート確定、`M-10`の一時プロファイル、`M-12`の既存プロファイル非参照、`M-13`の折返し通信、`M-15`〜`M-18`の秘密情報の扱い、`M-19`の送信前停止は、契約テストまたは3 OS統合テストで確認します。

#### 6.2.6. VMと物理端末と実サービスの分担

| 証明する範囲 | 使う実体 | 対象 |
|---|---|---|
| プロセス、ファイルシステム、プロファイル複製、秘密情報保管庫 | 対象OSのVMまたはself-hosted runner。可視デスクトップセッションと実OSサービスを備える | `M-01`〜`M-04`、`M-08`、`M-10`〜`M-18` |
| 公式拡張機能配布基盤の標準追加と、Chrome更新の前後 | 対応版の通常Google Chromeを入れた代表端末、または同等のfull desktop VM | `M-05`〜`M-07`、`M-09` |
| AtCoderログイン、Cloudflare、一往復UX、提出確認画面 | 通常Chromeを自動制御しない代表環境。**物理端末を少なくとも最終確認へ含める** | `M-20`、`M-21` |

headless CI、Chrome for Testing、WebDriverは回帰テストへ使えますが、`M-05`〜`M-07`と`M-20`・`M-21`の合格根拠にしません（§6.1）。環境そのものの確保手段は`TD-18`が決めます。

#### 6.2.7. 確認手段を用意できない組み合わせ

| 組み合わせ | 現状 | 扱い |
|---|---|---|
| `ENV-LIN-X64-NOSS`の`M-15` | Secret Serviceがない構成ではセッションを保存できない | MVP保証範囲の変更提案にしない。§4.1の契約どおり提出だけを停止し、`M-16`で確認する |
| `ENV-WIN-ARM64`、`ENV-LIN-ARM64` | 通常Google Chromeの公式提供の有無が未確認 | `TD-31`で確認する。提供がなければ[MVPスコープ §7](../product/mvp.md#7-変更管理)の変更管理へ、保証範囲からarm64を外す提案として回す |
| `ENV-MAC-X64` | 現在持っている端末はarm64だけで、x86_64のmacOSを同等環境として用意できるかが未確認 | `TD-18`で確保手段を決める。用意できなければ、macOSの保証をarm64に限る提案としてMVPスコープ §7 の変更管理へ回す |
| Linuxの`M-01`〜`M-18` | native Linuxの物理端末がない。**WSLは代替にならない** | デスクトップ付きのLinux VMを用意できるかを`TD-18`で確かめる。§6.1の同等環境の条件を満たせば証明に使える |
| Linuxの`M-20`・`M-21` | 実サービスsmokeの最終確認を物理端末で行えない | VMを代表環境として認めるか、Linuxの保証範囲を変更提案へ回すかを`TD-18`で決める |
| Linuxの`M-17`と表示文言 | 上の2行が解けるまで観測できない | `TD-48`で観測する。揃うまで表示文言を確定しない。未決事項 7.3 を未決のまま保持する |
| `M-09`のうち拡張機能の更新 | 更新テスト用の版が未uploadで、`V-12C`の必須caseから外れている（[ADR-0009](../decisions/0009-required-cases-for-v12c.md)） | MVP完了条件の確認としては残し、`TD-38`のライフサイクル契約と同じ変更で扱う |

**WSLをnative Linuxの代わりに使いません。** 理由は2つあります。第一に、[可搬性設計 §3.2](language-and-platform-portability.md#32-host-os)がWSLをMVPの対象外と定めています（「Linux kernelで動作しても、Windows filesystem・実行file・browser・credentialとの相互運用を未検証のまま保証しない」）。[MVPスコープ](../product/mvp.md#5-mvp完了条件)もnative WindowsをWSLと同一視しません。**WSLで得た結果は、native Linuxの結果にもnative Windowsの結果にもなりません。** 第二に、`M-01`〜`M-04`と`M-15`〜`M-17`が見ているのはLinux kernelとhost OSをつなぐ境界そのもので、WSLはそこが変わります。§6.1が同等環境へ求めるdesktop session、保管庫service、通常Google Chromeを既定で備えていない点も同じ理由に含まれます。
