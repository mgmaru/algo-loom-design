...
`V-12C`の**必須case**を次のとおり定めます。`V-12E`の事前条件と`V-12C`の合格条件が指すのはこの範囲です。

| # | 必須case | 検証層 |
|---:|---|---|
| 1 | 初回同意、拡張機能追加、browser起動、Cookie受領、本人確認、secret store保存、認証後遷移の各境界で、取消、timeout、browser終了、helper異常終了を安全側停止にできる | 契約test |
| 2 | 対象版、権限、配布元の不一致を検出して停止する | 契約test |
| 3 | 基準templateの破損を検出し、読取り専用の使い捨て複製で扱う | 契約test |
| 4 | secret store障害を分類し、提出前に停止する | 契約test |
| 5 | process、待受、file lockの競合を分類し、必要なら次回だけ初回設定へ戻す | 契約test |

いずれも**代表環境**で実行します。次の3つは必須caseに含めません。

| 含めないcase | 理由 | 扱う作業 |
|---|---|---|
| Chromeの更新前後 | 更新導線は`V-12E`の経路に現れない | `V-12C`へ後から足す |
| 拡張機能の更新（対象版から更新先版へ） | 同上。無効化表が更新版の組を独立caseとして切り出している | `V-12C`へ後から足す。upload に明示承認が要る |
| native Linux、native Windowsの3 OS統合test | 無効化表が結果を環境ごとに切り分けている。代表環境の結果は他環境に依存しない | [`TD-12`](../../TODO.md#td-12-3つのosの認証検証マトリクスを作る) |

**したがって`V-12`の合格は、代表環境で方式Aの一往復UXが成立することを示します。3つのOSでの成立と更新導線の成立は示しません。** 判断の経緯は[ADR-0009](../decisions/0009-required-cases-for-v12c.md)を参照します。
