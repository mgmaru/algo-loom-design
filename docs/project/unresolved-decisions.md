...
**残る判断:** **Linux**で実際に成立する保護範囲と、3つのOSを踏まえた利用者への表示文言。macOSは2026年9月20日、Windowsは2026年9月21日に観測済み。

**2026年9月20日に観測したこと（macOS）:** [AtCoder認証設計 §4.1.1](../architecture/atcoder-authentication.md#411-macosで観測した実際の保証範囲)に記録した。要点は3つ。

- 読み出しを許可される実行ファイルとして記録されるのは、**AlgoLoomのコードではなくPythonのインタプリタ**だった。[§3.6](../architecture/atcoder-authentication.md#36-配布物と実行ファイル署名の境界)の契約から導いていたとおり。
- そのため、**同じインタプリタを起動できる任意のプロセスが確認画面なしで値を読めた。** AlgoLoomの更新では再認可を求められない。ad-hoc署名の専用実行ファイルにした場合は逆に、再ビルドのたびに読めなくなり再認可が要る。
- **アクセス制御の信頼実行ファイル一覧は、汎用インタプリタを締め出さなかった。** ad-hoc署名の実行ファイルが作成し、読み出し許可にその実行ファイル1件だけを持つ項目を、Apple署名のPythonが読み出せている。仕組みは特定できていない。

いずれも「同じ利用者として動作する他のプロセスから読めないこと」を保証しない、という既存の記述を裏付ける。**macOSだけの観測で表示文言は確定しない。**

**2026年9月21日に観測したこと（Windows）:** [AtCoder認証設計 §4.1.2](../architecture/atcoder-authentication.md#412-windowsで観測した実際の保証範囲)に記録した。macOSと同じ観測IDを使い、5つの結果を並べて比較できる。**観測したのは`ENV-WIN-X64`だけで、`ENV-WIN-ARM64`は未観測である。** 要点は3つ。

- Credential Managerの`CRED_TYPE_GENERIC`の項目は、**作成した実行ファイルをアクセス制御として保持しない。** 利用者のアカウントへ紐づく。
- そのため、**別の実行ファイル（`powershell.exe`）が確認画面なしで値を読めた。** AlgoLoomの更新でも再認可を求められない。専用の実行ファイルを用意しても紐付ける先がなく、保護は変わらない。
- **macOSとは仕組みが違うのに、結論は同じになった。** macOSは実行ファイル単位のアクセス制御を持ちながら汎用インタプリタを締め出さず、Windowsはアプリ単位の紐付けをそもそも持たない。

**この観測は、観測物の不具合を直してから採った。** 1回目の実行は`other-executable`を「読めなかった」と報告したが、原因は64 bitでの構造体の読み出し位置の誤りであり、Windowsの保護ではなかった。経緯は[§4.1.2.1](../architecture/atcoder-authentication.md#4121-最初の実行が反対の結果を報告したこと)に残した。**保証範囲の観測では、読めるものを「読めなかった」と記録することが最も危険である。** その上に過大な表示文言を置いてしまうため。

**2つのOSで結論が一致したが、表示文言はまだ確定しない。** Linuxの観測（[`TD-48`](../../TODO.md#td-48-linuxの検証環境を確保し秘密情報保管庫が保証する範囲を観測する)）が揃っていないうえ、一致は「いちばん弱いOSへ合わせるか、OSごとに表示を変えるか」の判断を省く理由にならない。

**この2件は合否ではなく観測である。** [観測手順 §1](../verification/secret-store/README.md#1-何を確かめるか)のとおり「どの結果も『正解』ではない」。OSが実際に何を守るかを見た結果そのものが、表示文言の材料になる。

**決めること:** Linux Secret Serviceで、同じ2点（「同じ利用者として動作する他のプロセスから読めるか」「更新時に再認可を求められるか」）を確認し、3つのOSに対して過大でない表示文言を確定する。OSによって保護範囲が違う場合、いちばん弱いOSへ合わせるか、OSごとに表示を変えるかを決める。

**原文:** 「秘密情報保管庫が保証する範囲を次のとおり限定し、これを超える表示を行いません。」

出典: [AtCoder認証設計 §4.1](../architecture/atcoder-authentication.md#41-保管先)、[`TD-12`](../../TODO.md#td-12-3つのosの認証検証マトリクスを作る)、[`TD-47`](../../TODO.md#td-47-windowsの秘密情報保管庫が保証する範囲を観測する)（Windowsの観測）、[`TD-48`](../../TODO.md#td-48-linuxの検証環境を確保し秘密情報保管庫が保証する範囲を観測する)（Linuxの観測）、[`TD-46`](../../TODO.md#td-46-検証マトリクスを確定しwindowsとlinuxの秘密情報保管庫を観測する)（表示文言の確定）
