...
**残る判断:** **WindowsとLinux**で実際に成立する保護範囲と、3つのOSを踏まえた利用者への表示文言。macOSは2026年9月20日に観測済み。

**2026年9月20日に観測したこと（macOSのみ）:** [AtCoder認証設計 §4.1.1](../architecture/atcoder-authentication.md#411-macosで観測した実際の保証範囲)に記録した。要点は3つ。
...
**決めること:** WindowsのOS保護領域とLinux Secret Serviceで、同じ2点（「同じ利用者として動作する他のプロセスから読めるか」「更新時に再認可を求められるか」）を確認し、3つのOSに対して過大でない表示文言を確定する。OSによって保護範囲が違う場合、いちばん弱いOSへ合わせるか、OSごとに表示を変えるかを決める。
