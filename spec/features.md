...
| `SUBMIT` | AtCoder側との互換性とアカウントを確認し、一件ずつ明示的に提出し、判定を取得時刻付きで記録する | 自動提出、無条件の再送、未確認のAtCoder側変更の自動受け入れ、判定の推測 | `F-SUBMIT-01`〜`05` | [§8](#8-認証提出判定) |
...
| `F-SUBMIT-03` | AtCoderへの提出 | 互換性確認済みの`PREPARED`な操作を一件だけ送信し、提出IDまたは送信状態不明を記録する。無条件の再送はしない | `F-SUBMIT-02`, `F-SUBMIT-05` | [§8.3](#83-提出状態f-submit-03) |
...
| R7 | 外部通信と記録を最小限にする | 利用者の提出操作にともなう確認、明示的な診断、または許可された低頻度の保守確認に限定し、無期限の常駐監視を行わない |
...
利用時の確認は、初回提出前、前回の確認から許容期間を過ぎた後、および明示的な診断時に行います。確認間隔の具体値は、AtCoderへの確認結果と実測を根拠に、[§17](#17-未決事項)の制限値として確定します。ハッシュが変わった場合、利用者にはAtCoder側の変更を検出したこと、提出を停止したこと、公式サイトから手動提出できること、AlgoLoomの互換性修正または確認済み版を確認する手順を示します。
...
| 問題取得・認証・提出・判定 | `JudgeAdapter` | AtCoder固有の取得、認証確認、互換性確認、言語解決、提出、判定 |
