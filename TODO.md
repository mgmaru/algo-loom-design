...
「決定」列は、その作業から生まれた[決定記録](docs/decisions/README.md)を指します。**ADRの番号はTODOのIDと一致しません。** 一つの作業が複数の決定を生み、一つの決定が複数の作業にまたがるためです（[ADR-0001](docs/decisions/0001-adopt-decision-records.md)）。決定記録は2026年9月18日に導入し、それ以前の決定へは遡及していないため、当面この列の多くは`―`です。相互リンクの食い違いは`node scripts/check-docs.mjs`が検出します。

作業を再開するときの運用手順は[作業ガイド](CLAUDE.md)にあります。着手可能な作業は`node scripts/todo-status.mjs`で確認できます。
...
**仮説を実測で否定した経験を残します。** 「303を書く前に押下を通知するため`Shutdown`が先回りする」という説明は、4条件×20回の再現ですべて303が届き、否定されました。**測らずに直していたら、人が15分操作する6回目で同じ場所に止まっていました。**

毎回、campaignの開始前に検証物へ触る作業をまとめて片付けています。
...
- 次に審査へ出す版（`0.1.2`）の審査期間には、同じ凍結を再適用する
...
| [`TD-47`](#td-47-windowsの秘密情報保管庫が保証する範囲を観測する) | 技術検証 | Windowsの秘密情報保管庫が保証する範囲を観測する | ― | 完了 | ― |
...
| [`TD-58`](#td-58-提出をbrowserで行う理由を1か所へまとめる) | 機能設計 | 提出をbrowserで行う理由を1か所へまとめる | ― | 未着手 | ― |
| [`TD-41`](#td-41-製品形態の拡張機能の審査条件と確認gateを確定する) | 設計判断 | 製品形態の拡張機能の審査条件と確認gateを確定する | `TD-38`, `TD-40` | 未着手 | ― |
| [`TD-57`](#td-57-方式aを製品として採用するかを決める) | 設計判断 | 方式Aを製品として採用するかを決める | `TD-12`, `TD-40` | 未着手 | [ADR-0015](docs/decisions/0015-do-not-confirm-method-a-product-adoption-on-v12-pass.md) |
...
#### `TD-45` campaign manifestの確定遷移が自分を無効化する不整合を直す
...
**この不整合は`TD-37`から存在していました。** `V-12B`を実機で最後まで通したのが9月20日が初めてで、それまで`manifest compare`をこの遷移へ適用する場面がなかったためです。
...
- [x] `TD-43`・`TD-44`とまとめて直したうえで、新しいcampaign IDで`V-12`をやり直す準備が整っている（2026年9月20日。campaign `v12-2026-09-20-02`を作り、`V-12A`が合格した）

---

#### `TD-49` `V-12E`の`submit`相当の入口を検証物へ足す
...
| 対象ファイル | [`scripts/verification/atcoder_v12/helper/`](scripts/verification/atcoder_v12/helper/) |
| 依存 | ― |
| 決定 | [ADR-0011](docs/decisions/0011-add-submit-entry-before-rerunning-v12.md) |

**なぜこの作業が要るか:** 2026年9月20日、3回目のcampaignで`V-12D`まで合格した後、`V-12E`へ進めないことが分かりました。**検証物に`submit`相当の入口がありません。** [検証計画の`V-12E`](docs/project/judge-adapter-verification.md#311-v-12-方式a製品形態の検証)は「再認証理由と中止方法の表示 → browser自動起動 → AtCoder login → **同じbrowserで対象・言語・sourceを示す提出確認画面**」を一連で要求し、手動のページ移動とcopy-and-pasteを停止条件にしています。現在のhelperは`serve`・`first-login`・`recheck`しか持たず、認証後は`https://atcoder.jp/settings`で終わります。

**この不足は`TD-37`から存在していました。** 1回目は`V-12D`の手前、2回目は`V-12C`の手前で止まったため、`V-12E`へ到達したのが3回目が初めてだったからです。

**手順:**

1. `submit`相当の入口をhelperへ足す。**拡張機能のsourceは変えない**（[ADR-0011](docs/decisions/0011-add-submit-entry-before-rerunning-v12.md)決定2。変えるとCWSの再審査が発生する）
2. 入口が、保存済みlocal sessionの不在を検出し、**再認証の理由と中止方法を先に表示する**ことを固定入力testで固定する
3. 認証成立後に、同じChromeで対象問題の提出確認画面まで遷移させる。**提出formを操作せず、最後の提出操作を行わない。** 対象・言語・sourceの提示方法を決め、`V-12E`の「提出前入力」として何を渡すかを固定する
...
#### `TD-50` helperの誤った成功報告と原因を指せないエラー名を直す
...
**なぜこの作業が要るか:** 2026年9月20日の`V-12C`の実行で、helperの報告が実体とずれる箇所を2件見つけました。**どちらも`V-12C`の必須caseは通っています。不合格ではなく、次に直す対象として起票します。**

| # | 観測 | なぜ困るか |
|---|---|---|
| 1 | `secret delete`へKeychain adapter以外の実行ファイル（`/bin/echo`）を渡すと、**何も消していないのに`{"ok":true,"secret_store_item_removed":true}`を返す。** 終了コード0を成功と解釈している | campaign終了時の後始末で使うcommandである。**秘密情報を消したつもりで残る。** [検証物README](scripts/verification/atcoder_v12/README.md)は`keychainItemAbsent`について「判定できなかったものを『項目あり』と報告しない」と書いているが、`delete`側に同じ扱いがない |
| 2 | `profile inspect`が、対象版が入っていない場合と導入が重複している場合の**両方**で`extension_installation_not_unique`を返す | 次に取る行動が違う。0件なら「対象itemと版を確かめる」、2件以上なら「重複した導入を片付ける」。[`TD-44`](#td-44-helperのエラーが原因を一意に指せない箇所を洗い出して直す)が`first-login`について直した問題と同型で、`profile.go`が残っていた |

**`TD-44`の本文と完了条件は書き換えません。** 完了した作業の記録を残し、見つかった残りを新しい作業として起票します。

**手順:**

1. `secret delete`が、adapterの同一性を確認できない場合と削除を確認できない場合を成功として報告しないようにする。**「消せた」と「判定できなかった」を分ける**
2. `profile inspect`の`extension_installation_not_unique`を、0件の場合と2件以上の場合で分ける。名前は「次に取る行動」で決める（[検証物README](scripts/verification/atcoder_v12/README.md)のエラー名の方針）
3. helperの残りの報告値についても、**確かめたい対象と報告している対象がずれていないか**を同じ観点で洗い出す（[`TD-45`](#td-45-campaign-manifestの確定遷移が自分を無効化する不整合を直す)手順3と同じ形）
...
#### `TD-51` 提出確認画面のform POSTがbrowserで拒否される問題を直す
...
| 対象ファイル | [`scripts/verification/atcoder_v12/helper/`](scripts/verification/atcoder_v12/helper/) |
| 依存 | ― |
| 決定 | [ADR-0012](docs/decisions/0012-serve-submission-page-with-same-origin-referrer-policy.md) |

**なぜこの作業が要るか:** 2026年9月20日の4回目のcampaignで、`V-12E`が最後の受け渡しで停止しました。提出確認画面の「AtCoderの提出画面へ進む」を押したPOSTを、**helper自身が`authentication_rejected`で拒否しました**（[実行記録 §8](docs/verification/judge-adapter/results/2026-09-20-v12-03.md#8-v-12eが最後の受け渡しに到達できなかった理由)）。画面が`Referrer-Policy: no-referrer`付きで配信されるため、そのページからのform POSTでChromeが`Origin: null`を送り、helperのorigin検査と一致しません。**`TD-11`はこの作業が終わるまで再開できません。**

**手順:**

1. 提出確認画面の応答の`Referrer-Policy`を`no-referrer`から`same-origin`へ変える。**`Origin`の検査は緩めない。** 同意画面の`no-referrer`は変えない（[ADR-0012](docs/decisions/0012-serve-submission-page-with-same-origin-referrer-policy.md)決定2）。
2. 同一originへのform POSTで`Origin`が正しく送られ、別originへの303遷移で`Referer`が送られないことを、**browserで実際に確かめる。** 手でヘッダーを立てたrequestを成立証拠にしない。
3. 契約testを、browserが実際に送る値で確かめる形へ直す。`Origin: null`を拒否することと、正しいoriginを受け付けることの両方をcaseにする。
4. [`TD-52`](#td-52-browser由来のrequestを手で組み立てている契約testを洗い出す)と**同じ変更にまとめる。** helperのbuild hashが動く回数を1回に抑えるためで、[ADR-0011](docs/decisions/0011-add-submit-entry-before-rerunning-v12.md)決定3と同じ理由。
5. 5回目のcampaignの開始前に、新しいhelperのbuild hashを記録する。

**完了条件:**

- [x] 提出確認画面の`Referrer-Policy`が`same-origin`になっている（2026年9月20日。`protocol.go`の`submissionReferrerPolicy`）
- [x] `Origin`の検査を緩めていない。`null`を受け付けていない（`Origin: null`を拒否するcaseを足した）
- [x] 同一originの`Origin`と別originの`Referer`を、browserで実測して確認している（[`browser-request-probe.mjs`](scripts/verification/atcoder_v12/browser-request-probe.mjs)。`same-origin`は`Origin`が保たれ、別originへ`Referer`を送らない）
- [x] 契約testが、手で立てたヘッダーではなくbrowserの実挙動と対応している（許可listをsourceの定数と共有し、probeが同じ定数を読む）
- [x] 同意画面の`Referrer-Policy`を変えていない
- [x] `TD-52`と同じ変更にまとまっている
- [x] 各caseのtestが、修正前のコードで落ちることを確認できている（`no-referrer`へ戻す変異と`null`を許す変異の2つで、いずれもGo testが落ちた）

**2026年9月21日に記録しました。** 5回目のcampaignの開始前に、cleanな`95b60bc`から別pathへ2回buildし、helperのbuild hashは`087038c7…`で一致しました。**dirtyな作業treeでは`campaign_ready`が`false`になり、そのbuildをcampaignへ使えません。**

---

#### `TD-52` browser由来のrequestを手で組み立てている契約testを洗い出す
...
**なぜこの作業が要るか:** [`TD-51`](#td-51-提出確認画面のform-postがbrowserで拒否される問題を直す)の不具合は、契約testが`Origin`ヘッダーを手で立てていたため見つかりませんでした。**[ADR-0005](docs/decisions/0005-verify-consent-flow-in-browser-semantics.md)と同じ型の見落としが2度目です。** あのときは「ソースの文字列検査だけを成立証拠にしない」としてcontent scriptを実際に評価する形へ変えました。今度は、**requestを手で組み立てている箇所**に同じ穴が残っていました。1箇所を直すだけでは、3度目が起きます。

**手順:**

1. helperとNodeの契約testから、**browserが送るはずのrequestを手で組み立てている箇所**をすべて挙げる。`Origin`、`Referer`、`Content-Type`、`Sec-Fetch-*`、Cookieの各ヘッダーを対象にする。
2. 箇所ごとに、その値が**browserの実挙動に依存するか**を判定する。依存しないもの（helperが自分で決める値、拡張機能が明示的に付ける値）は手で立ててよい。区別した根拠を書く。
3. 依存するものについて、ローカルだけで実挙動を確認する手段を用意する。**AtCoderにもChrome Web Storeにも接続しない。**
4. 見つかった食い違いは、`TD-51`と**同じ変更で**直す。食い違いが無かった箇所も「確認した」と記録する。**確認していないものを確認済みにしない。**
5. 実行して評価する範囲の定義（[ADR-0008](docs/decisions/0008-scope-of-execution-based-checks-for-verification-artifacts.md)）へ、この判定基準を反映するかを検討する。反映しない場合は理由を残す。

**完了条件:**

- [x] 手で組み立てているrequestの一覧があり、各箇所がbrowser依存かどうか判定されている（2026年9月20日。6箇所4種。[検証物README](scripts/verification/atcoder_v12/README.md)の分類表）
- [x] browser依存の箇所に、ローカルで実挙動を確認する手段がある（[`browser-request-probe.mjs`](scripts/verification/atcoder_v12/browser-request-probe.mjs)。negative controlを必ず測る）
- [x] 見つかった食い違いが`TD-51`と同じ変更で直っている（食い違いは`Origin`の1件。`Content-Type`は一致を実測で確認した）
- [x] 食い違いが無かった箇所も、確認した事実として記録されている（`Content-Type`、`Host`、接続元、拡張機能originの扱いを分類表に記載）
- [x] 外部サービスへ接続していない（probeはloopbackだけを使う。使い捨てChrome profileはリポジトリ外に作り破棄する）
- [x] ADR-0008へ反映するかを判断し、結論を記録している（**新しいADRを追加しない。** 判定基準は[ADR-0012](docs/decisions/0012-serve-submission-page-with-same-origin-referrer-policy.md)決定3が定め、運用上の規則は検証物READMEに置く。ADRは判断時点の記録であり、日々参照する規則の置き場ではないため）

**この規則をREADMEへ置いた理由:** 同じ型の失敗が[ADR-0005](docs/decisions/0005-verify-consent-flow-in-browser-semantics.md)に続いて2度目でした。ADRは判断の記録で書き換えないため、**検証物へ触る人が最初に読む場所に規則として置かないと、3度目を防げません。**

---

#### `TD-53` 提出確認画面から提出pageへの受け渡しが成立しない原因を特定する
...
**最初の仮説は実測で否定済みです。** 「303を書く前に押下を通知するため`Shutdown`が先回りする」という説明は、Goの`http.Server.Shutdown`が処理中のrequestを待つため成立しません（4条件×20回で、すべて303が届いた）。**否定された仮説に基づく修正を行いません**（[ADR-0013](docs/decisions/0013-find-the-cause-before-fixing-the-v12e-handoff.md)決定1）。

**手順:**

1. 症状を**ローカルで再現する**。提出確認画面と同じ配信条件（`Referrer-Policy: same-origin`、JavaScriptなし、同一originへのform POST、押下後に待受を閉じる）を実際のChromeへ与え、browserが303を受け取って遷移し終えるかを測る。**AtCoderにもChrome Web Storeにも接続しない。**
2. **negative controlを必ず入れる。** 受け渡しが壊れている条件（例: 応答を書かずに待受を閉じる）で`ERR_CONNECTION_REFUSED`相当を検出できることを先に示す。検出できない測り方を成立証拠にしない（[ADR-0012](docs/decisions/0012-serve-submission-page-with-same-origin-referrer-policy.md)決定3と同じ規則）。
3. 再現したら、**症状を説明できる仮説を立ててから**直す。再現しない場合は、実機のcampaignで何をどこに記録するかを先に決める（[ADR-0013](docs/decisions/0013-find-the-cause-before-fixing-the-v12e-handoff.md)の再評価条件）。
4. 直した後、probeと固定入力testの両方で確認する。**修正前のコードでtestが落ちることも確かめる。**
5. [`TD-54`](#td-54-helperの成功報告が到達していないことを覆い隠さないようにする)と**同じ変更にまとめる。** helperのbuild hashが動く回数を1回に抑えるため。
6. 6回目のcampaignの開始前に、新しいhelperのbuild hashを記録する。
...
#### `TD-54` helperの成功報告が「到達していない」ことを覆い隠さないようにする
...
1. `submit-entry`の結果JSONを、**helperが観測できた範囲と、できていない範囲が区別できる形**にする。`ok`の意味を「helperが観測できた範囲が成立した」に限定する。
2. **到達を人の観測として記録する項目を用意する。** helperが自分で埋めない。埋まっていないことが分かる初期値にする。
3. helperの出力だけで`V-12E`の合否を述べられないことを、[検証物README](scripts/verification/atcoder_v12/README.md)へ書く。
4. 同じ構造が他のcommandの結果JSONにもないかを確認する。**観測できない範囲を`true`で覆っている箇所**を対象にする。
5. [`TD-53`](#td-53-提出確認画面から提出pageへの受け渡しが成立しない原因を特定する)と**同じ変更にまとめる。**
...
**2026年9月20日の実施記録（3回目、campaign `v12-2026-09-20-02`。実施済み）:** 手順1〜4を実行し、**`V-12A`が外部通信0件で合格しました**（[実行記録](docs/verification/judge-adapter/results/2026-09-20-v12-02.md)）。2回目の資源（基準template、検証用secret store項目、manifest、実行script、`build/`）を先に破棄し、secret store項目が残っていないことを確認してから作り直しています。**やり直しの前提だったhelperのbuild hashは、`f6caf3d`と`018ba98`の別々のcommitからbuildして`637a1928…`で一致しました。** 拡張ZIP、Keychain adapter、review bundleもbyte単位で一致し、[ADR-0007](docs/decisions/0007-make-helper-build-hash-track-behaviour.md)の意図どおりhashが挙動へ追従しています。配信中の`0.1.1`は取得済みのbytesがmanifestの`signed_builds`と一致し、`atcoder.js`・`bootstrap.js`・`service_worker.js`・`icon128.png`が生成物とbyte単位で一致、権限は`cookies`・`storage`とhost 2件のみでした。固定入力のtestはGo 14件、Node 20件、review fixture 16 caseがすべて合格しています。明示承認を得たうえで手順5〜7を実行し、**`V-12B`・`V-12C`の必須case・`V-12D`も合格しました。** 標準追加から基準templateの一度だけの確定（完全性ID `b7612eab…`）、同意画面、AtCoderログイン、本人照合、secret store保存、新processからの再照合までが分断なく成立しています。`GET /settings`は**上限と同数の2回**、提出0件、Bot対策の回避0件です。**2回目のcampaignを止めた不整合も解消を確認しました。** 基準templateの確定でmanifestをrevision 2へ上げても`manifest compare`の`invalidated`は空で、`profile_contract_established`と判定されました（[ADR-0006](docs/decisions/0006-profile-contract-establishment-is-not-invalidation.md)）。

**`V-12E`は実行できませんでした。検証物に`submit`相当の入口が無いためです。** 不合格ではなく、確かめる手段がありません。手で代用すると`V-12E`の停止条件（手動ページ探索）に該当するため行いません。**したがって`V-12`全体は未合格です。** 対応は[ADR-0011](docs/decisions/0011-add-submit-entry-before-rerunning-v12.md)で決め、[`TD-49`](#td-49-v-12eのsubmit相当の入口を検証物へ足す)と[`TD-50`](#td-50-helperの誤った成功報告と原因を指せないエラー名を直す)を起票しました。**この2件を同じ変更で直すとhelperのbuild hashが変わり、このcampaignの合格も取り消されます。** 4回目のcampaignで`V-12A`からやり直します。

**`V-12C`の実行中に、helperの報告が実体とずれる箇所を2件見つけました**（`secret delete`の誤った成功報告と、`profile inspect`のエラー名）。どちらも必須caseは通っており、`TD-50`で直します。
...
| ~~Windowsの秘密情報保管庫の観測~~ | **2026年9月21日に[`TD-47`](#td-47-windowsの秘密情報保管庫が保証する範囲を観測する)で完了**（[認証設計 §4.1.2](docs/architecture/atcoder-authentication.md#412-windowsで観測した実際の保証範囲)） | ― |
...
| 3つのOSを踏まえた表示文言 | Linuxの観測が揃うこと | [`TD-46`](#td-46-検証マトリクスを確定しwindowsとlinuxの秘密情報保管庫を観測する) |
...
2. 秘密情報保管庫を、macOS Keychain、Windowsの保護領域、Linux Secret Serviceとして明示する。あわせて[未決事項 7.3](docs/project/unresolved-decisions.md#73-秘密情報保管庫が保証する範囲と表示文言)に従い、各OSで「同じ利用者として動作する他のプロセスから読めるか」「AlgoLoomの更新時に再認可を求められるか」を確認し、[認証設計 §4.1](docs/architecture/atcoder-authentication.md#41-保管先)の保証範囲と利用者への表示文言を確定する。
   観測には[`scripts/verification/secret_store_guarantees.py`](scripts/verification/secret_store_guarantees.py)を使う。**macOSは2026年9月20日**（[認証設計 §4.1.1](docs/architecture/atcoder-authentication.md#411-macosで観測した実際の保証範囲)）、**Windowsは2026年9月21日に実施済み**（[認証設計 §4.1.2](docs/architecture/atcoder-authentication.md#412-windowsで観測した実際の保証範囲)）。**Windowsは[`TD-47`](#td-47-windowsの秘密情報保管庫が保証する範囲を観測する)、Linuxは[`TD-48`](#td-48-linuxの検証環境を確保し秘密情報保管庫が保証する範囲を観測する)へ分けた。** 止まっている理由が違うためで、Windowsは依存がなく、Linuxは`TD-18`待ちである。表示文言の確定は3つが揃ってから`TD-46`で行う。
...
- [ ] 3 OSそれぞれで秘密情報保管庫が実際に保証する範囲を観測し、過大でない表示文言が確定している（macOSとWindowsは観測済み。Linuxは`TD-48`、表示文言の確定は`TD-46`へ分けた）
...
| カテゴリ | 技術検証 |
| 状態 | **完了**（2026年9月21日） |
| 対象ファイル | [`docs/architecture/atcoder-authentication.md`](docs/architecture/atcoder-authentication.md) §4.1.2、[`docs/project/unresolved-decisions.md`](docs/project/unresolved-decisions.md) 7.3、[`scripts/verification/secret_store_guarantees.py`](scripts/verification/secret_store_guarantees.py) |
| 依存 | ― |
...
- [x] Windows実機で5つの観測すべてに結果がある
- [x] `same-interpreter`と`other-executable`の結果が、macOSと並べて比較できる形で記録されている（同じ観測IDを使い、§4.1.1と同じ並びにした）
- [x] 観測に使った項目がCredential Managerに残っていない（観測物の`residue`と、probe専用の接頭辞へ絞った`CredEnumerateW`の両方で確認した）
- [x] 記録に端末名、利用者名、絶対パスが含まれていない
- [x] 表示文言の確定を含めず、`TD-46`へ渡している
...
| ~~Windowsの`M-17`~~ | **解けた。** [`TD-47`](#td-47-windowsの秘密情報保管庫が保証する範囲を観測する)で2026年9月21日に観測した（[認証設計 §4.1.2](docs/architecture/atcoder-authentication.md#412-windowsで観測した実際の保証範囲)） |
...
| 3つのOSを踏まえた秘密情報保管庫の表示文言 | Linuxの観測が揃うこと。**この作業で決める** |
...
2. 3つのOSの観測が揃っていることを確認する。**観測そのものはこの作業で行わない。** macOSは[認証設計 §4.1.1](docs/architecture/atcoder-authentication.md#411-macosで観測した実際の保証範囲)、Windowsは[§4.1.2](docs/architecture/atcoder-authentication.md#412-windowsで観測した実際の保証範囲)にあり、Linuxは`TD-48`が扱う。揃っていなければ先にそちらを終わらせる。
...
| 依存 | `TD-11`、`TD-12` |
| 参照 | [拡張機能の責任境界と提出の縮退運転設計 §2](docs/features/extension-boundary-and-degraded-submission.md#2-拡張機能の責任境界)、[同 §4.2](docs/features/extension-boundary-and-degraded-submission.md#42-td-38へ渡す)、[Chrome拡張機能配布の運用リスクと緩和策 §2](docs/project/chrome-extension-distribution-risks.md#2-リスク1-更新の非対称性) |
...
2. 互換性規則を決める。どの組み合わせを起動可能とし、版不一致、想定外の権限、配布・審査状態不明、Chrome更新後の完全性不成立で、再利用、再設定または安全側停止のどれを選ぶかを表にする。未知の新しい版を自動で信頼しない。次を前提にする（[配布リスク §2.4](docs/project/chrome-extension-distribution-risks.md#24-緩和策の候補)）。
   - **固定する対象**は拡張機能ID、権限セット、配布元。**交渉する対象**は折返し通信protocol版。拡張機能の厳密な版は固定しない。厳密な版固定は`V-12`の検証要件であり、製品要件ではない。
   - 拡張機能は常に先に更新されるため、**後方互換の責任を拡張機能側へ置く**。拡張機能は現行protocol版とその一つ前を話せるようにし、予告なく古いprotocolを落とさない。
   - **権限セットの変更は修正releaseではなく設計変更として扱う**。Chromeでは要求権限が増えると利用者の再承認が必要になり、承認まで機能が停止する。
3. 拡張機能の責任境界を契約へ組み込む（[設計 §2](docs/features/extension-boundary-and-degraded-submission.md#2-拡張機能の責任境界)）。拡張機能側で固定する対象page、実行できる操作の種類、受け付ける要素指定の検証規則、要求権限を確定し、認証helperから変更できないことを明記する。要素指定の受け渡し形式もここで決める。アカウント照合を認証helperへ一本化する。
4. 初回作成、正常再利用、拡張機能自動更新、helper更新、protocol更新、権限・取得対象・用途・保存先の変更、テンプレート破損、配布停止、利用者による認証情報削除の状態遷移を定義する。
5. 再同意と再設定の境界を固定する。同じ同意版と権限で健全なテンプレートを再利用するときは繰り返さず、取得対象、用途、保存先、権限が変わる場合は認証開始前に再同意を求める。標準追加画面以外の強制導入へ切り替えない。
6. 削除の意味を分ける。保存済みsessionの削除、テンプレートと実行用profileの削除、拡張機能の端末内削除、AtCoder server上のsession失効を同一操作として表示しない。AlgoLoom所有領域を削除した後も利用者の既存Chromeへ触れない。
7. 更新中断とrollbackの契約を決める。互換な旧版へ戻せる条件、戻せない場合の停止、原子的に置き換えるmetadata、途中状態の回収、次回開始時の診断を含める。CWSのrollbackは旧版を新しい版番号で再公開する操作であり、それも自動更新で全利用者へ配られることを前提にする。
8. 事前検知に使うテンプレート検査の項目と実行コストを確定する（[設計 §3.2](docs/features/extension-boundary-and-degraded-submission.md#32-事前に検知する)）。`submit`が外部通信前に提出準備の自動化可否を判定できる項目に限定する。
9. 利用者が残っている状態での配布停止手順を決める。Chrome 117以降、掲載が失われるとインストール済みの利用者へ警告が表示されるため、unpublishを既定の停止手段にしない（[配布リスク §3.3](docs/project/chrome-extension-distribution-risks.md#33-停止手段としてのunpublishに副作用がある)）。
10. 拡張機能とAlgoLoom本体のrelease順序を決める。拡張機能はChrome Web Store、本体はPyPIという別経路で配られ、審査の所要時間も異なる。AlgoLoomは審査済みの版を固定して照合するため、新しい拡張機能の版を要求する本体を、その版の審査通過前に公開しない条件を明記する（[配布方針ガイド §16](docs/operations/algoloom-distribution.md#16-段階的な配布計画)）。
11. 正常系、版不一致、権限変更、完全性不成立、削除、更新中断について、機能要件と受け入れシナリオへ投影する内容を`TD-15`へ渡す。配布・build・release手順へ渡す要件を`TD-35`へ対応付ける。
...
- [ ] 固定する対象と交渉する対象が分かれ、拡張機能の厳密な版に依存していない
- [ ] 拡張機能側で固定する対象page、操作の種類、要素指定の検証規則、要求権限が確定している
- [ ] 事前検知に使うテンプレート検査の項目が確定している
...
#### `TD-58` 提出をbrowserで行う理由を1か所へまとめる

| 項目 | 内容 |
|---|---|
| カテゴリ | 機能設計 |
| 対象ファイル | [`docs/architecture/atcoder-authentication.md`](docs/architecture/atcoder-authentication.md)（節の新設）、[`docs/features/extension-boundary-and-degraded-submission.md`](docs/features/extension-boundary-and-degraded-submission.md) |
| 依存 | ― |

**なぜこの作業が要るか:** 2026年9月21日、「`REVEL_SESSION`を取得した後の提出をCLIで完結できないのはなぜか」という問いに答えるため、**4か所を突き合わせる必要がありました。**

| 場所 | そこにある内容 |
|---|---|
| [認証設計 §8](docs/architecture/atcoder-authentication.md#8-採用しない前提と方式) | Turnstileの自動突破・回避と、直接HTTP提出へのfallbackを採らない |
| [拡張機能の責任境界 §3.8](docs/features/extension-boundary-and-degraded-submission.md#38-採用しない選択肢) | sessionを用いた直接HTTP提出は、Turnstileを回避することになる |
| [配布判断記録 #2・§3.2](docs/project/atcoder-public-policy-review.md) | AtCoderの告知（非公式toolは非サポート、`submit`等へCAPTCHA導入）と、8項目の条件 |
| [`p0-12`](docs/verification/judge-adapter/results/2026-08-12-p0-12.md) | **提出フォームにCSRF欄とTurnstileを各1件観測した**（2026年8月12日） |

**中心にある事実が1行で書かれていません。** 「提出フォーム自体がTurnstileで守られているため、`REVEL_SESSION`だけではPOSTできない」という観測事実と、そこから導かれる設計上の帰結が分かれて置かれています。同じ問いは繰り返し出ます。

**手順:**

1. 認証設計へ、提出をbrowserで行う理由を述べる節を新設する。**新しい判断を足さない。** 既にある判断と観測の集約にとどめる
2. 次の4つをこの順で書く。**観測事実、AtCoderの公開情報、AlgoLoomの不採用条件、CLIで完結している範囲**
3. 「CLIで完結している範囲」を表にする。問題取得、ローカルtest、本人照合、秘密情報保管庫への保存、提出フォームへの値の設定、提出IDの取得、判定確認を並べ、**利用者がbrowserで行うのはログインと最後の提出操作だけ**であることを示す
4. 既存の3か所からは移動させず、**新しい節へのリンクを足す**。正本の位置を変えない
5. `p0-12`の観測日を明記する。**2026年8月12日の観測であり、現在のフォームを再確認したものではない**ことが分かる形にする。再確認は[`TD-40`](#td-40-提出ページのcontent-scriptとturnstileの共存を検証する)が行う

**完了条件:**

- [ ] 提出をbrowserで行う理由が1か所で読める
- [ ] 観測事実、公開情報、不採用条件、CLIで完結している範囲が区別されている
- [ ] 新しい判断を足していない（既存のADRと正本の内容を超えていない）
- [ ] 観測日が明記され、再確認の担当が`TD-40`であることが分かる
- [ ] 既存の3か所の正本の位置を変えていない
...
#### `TD-57` 方式Aを製品として採用するかを決める

| 項目 | 内容 |
|---|---|
| カテゴリ | 設計判断 |
| 対象ファイル | [`docs/architecture/atcoder-authentication.md`](docs/architecture/atcoder-authentication.md) §0、`docs/decisions/`配下の新しい決定記録 |
| 依存 | `TD-12`, `TD-40` |
| 決定 | [ADR-0015](docs/decisions/0015-do-not-confirm-method-a-product-adoption-on-v12-pass.md) |

**なぜこの作業が要るか:** [ADR-0015](docs/decisions/0015-do-not-confirm-method-a-product-adoption-on-v12-pass.md)決定2は、方式Aの製品採用を`TD-12`と`TD-40`の結果を見てから決めると定めました。**その「決める」作業がどのTODOにも無く、`todo-status.mjs`にも現れていませんでした。** 材料が揃っても誰も決めに来ない状態になるため、作業として起こします。

[認証設計 §0](docs/architecture/atcoder-authentication.md#0-結論)の3は、いまも「`V-12`で確認するまで製品採用を確定しません」と書いています。**`V-12`は2026年9月21日に合格しました**（[`v12-05`](docs/verification/judge-adapter/results/2026-09-21-v12-05.md)）。条件は満たされており、残るのは人の判断です。

**この判断は製品の契約を変えます。** 利用者へ示す約束と外部作用の範囲に関わるため、[作業ガイド §8](CLAUDE.md#8-判断の分担)に従い、決める前に提示して確認を得ます。

**手順:**

1. `TD-12`（3つのOSでの成立）と`TD-40`（提出ページでのcontent scriptとTurnstileの共存）の結果を並べる。**どちらも未完了なら着手しない**
2. 採用する場合に確定する範囲と、確定しない範囲を先に書き出す。`V-12`の合格が示した範囲は[認証設計 §3.1.6](docs/architecture/atcoder-authentication.md#316-v-12の実機検証で成立した範囲)にある
3. 採否を決め、**同じ変更で決定記録を書く。** 採用する場合は[認証設計 §0](docs/architecture/atcoder-authentication.md#0-結論)の3を書き換え、[ADR-0015](docs/decisions/0015-do-not-confirm-method-a-product-adoption-on-v12-pass.md)へ状態と行き先のリンクを足す
4. 採用しない場合は、**回避策を足さず**、方式Aの再設計または`TD-09`の別候補へ戻す。戻す先と理由を記録する
5. 採用する場合、[`TD-41`](#td-41-製品形態の拡張機能の審査条件と確認gateを確定する)と[`TD-38`](#td-38-認証配布物とテンプレートのライフサイクル契約を確定する)へ渡す条件を書く

**完了条件:**

- [ ] `TD-12`と`TD-40`の結果を材料として示している
- [ ] 採否が決まり、決定記録に背景・前提・採らなかった案・再評価条件がある
- [ ] 採用する場合、認証設計 §0 が更新され、確定した範囲と確定していない範囲が区別されている
- [ ] 採用しない場合、回避実装ではなく再設計または別候補へ戻す記録がある
- [ ] 決定の前に人へ提示し、確認を得ている
...
**2026年8月27日の完了記録:** **保存すると決定しました。** ただし7項目と並ぶ8番目の履歴項目としてではなく、**SolveAttemptに付随する観測**として保存します。§5.1の7項目は「利用者が何をしたか」の記録であり、ツールチェーン観測は「観測が生まれた文脈」で性質が異なるためです。保存するのは種類、自己申告のversion文字列、host OS名とversion、CPU architectureに限り、絶対path、hostname、利用者名、環境変数、`PATH`は保存しません。canonical language IDを識別子とし、観測を識別子や結合keyにしません。私的exportへ含め、公開用solution bundleへ含めません。MVPは端末ローカル保存とし、同期は「条件付き」を維持します。

判断の根拠は、可搬性設計 §9.1 が既に「診断・再現性の補助情報」として分類していること、[解き直しworkflow設計 §6.2](docs/features/revisit-workflow.md)の「同じalgorithmの別言語」比較が既に依存していること、`F-BASE-01` R4で既に観測しており追加の外部作用がないこと、そして**ツールチェーンの変化を示せないと環境差を成長と誤読させる**ことです。[Core契約 §5.1](docs/architecture/core-contracts.md#51-mvpで保存する履歴)を範囲の正本とし、可搬性設計 §4.2・§9.1 と未決事項 2.6 を同じ変更で一致させました。
...
|---|---|
| カテゴリ | 機能設計 |
| 対象ファイル | [`spec/features.md`](spec/features.md) §8.1、§8.2、§8.3、§8.4、§2.2、§15、[`spec/core-contracts.md`](spec/core-contracts.md) |
| 依存 | `TD-01`、`TD-38` |
| 参照 | [拡張機能の責任境界と提出の縮退運転設計 §3](docs/features/extension-boundary-and-degraded-submission.md#3-提出の縮退運転)、[同 §4.1](docs/features/extension-boundary-and-degraded-submission.md#41-td-15へ渡す)、[Chrome拡張機能配布の運用リスクと緩和策](docs/project/chrome-extension-distribution-risks.md) |
...
8. 受け入れシナリオ §15 へ、少なくとも次を独立して確認できるシナリオを追加する。番号は既存の末尾から連番にする。
   ...
   - 拡張機能が非互換または不在のとき、ブラウザを開く前に理由と一手を示し、明示操作があった場合だけ縮退運転で提出する。
9. §8.2 の提出前確認へ、**外部通信前に提出準備の自動化可否を判定する**項目を追加する。拡張機能の有無、版、権限、テンプレートの完全性、通信protocolの互換性を、ブラウザを起動する前にAlgoLoom所有のテンプレートから判定する（[設計 §3.2](docs/features/extension-boundary-and-degraded-submission.md#32-事前に検知する)）。
10. 縮退運転を機能要件へ投影する（[設計 §3](docs/features/extension-boundary-and-degraded-submission.md#3-提出の縮退運転)）。次を含める。
    - §8.3 へ、縮退運転による提出を区別して記録する要件を追加する。**新しい提出状態は追加しない**。`L2`相当は既存の`REMOTE_STATUS_UNKNOWN`と既存の回復経路を使う。
    - 縮退運転が利用者の**新しい明示操作**として始まり、自動でフォールバックしないことを要件にする。
    - `L1`（提出準備だけの縮退）と`L2`（認証ごとの縮退）を区別し、`L1`では提出IDの取得と判定確認を通常経路で行うことを明記する。
11. `F-SUBMIT-01` R2・R5 へ、[設計 §3.4](docs/features/extension-boundary-and-degraded-submission.md#34-停止理由から原因分類と次の一手への写像)の写像を投影する。**「利用者が直せるか」で示す一手を分ける**こと（AlgoLoom本体の更新を促す／標準追加へ導く／縮退運転を提示する／停止する）を要件にする。通知は[設計 §3.5](docs/features/extension-boundary-and-degraded-submission.md#35-利用者への通知)の4要素（何ができないか、何は無事か、今回どうするか、恒久的にどうするか）を満たし、推奨を一つ・代替を一つ示す。
12. §17 の認証未決事項を更新し、`aloom auth login`と一往復UXを未決へ戻さず、状態確認・削除commandの名称、具体表示、対応browser matrixだけを残す。

**完了条件:**

- [ ] 提出準備の自動化可否を外部通信前に判定する要件がある
- [ ] 縮退運転が明示操作として始まり、新しい提出状態を追加せずに記録できる
- [ ] 停止理由から原因分類と一手への写像があり、利用者が直せる問題と直せない問題で示す一手が分かれている
- [ ] 通知の4要素が受け入れシナリオへ落ちている
...
| 依存 | `TD-08`、`TD-15` |
| 参照 | `F-SUBMIT-05`、`E2E-31`、`E2E-32`、[拡張機能の責任境界と提出の縮退運転設計 §4.3](docs/features/extension-boundary-and-degraded-submission.md#43-td-36へ渡す) |
...
6. 提出formの要素指定をAlgoLoom本体側の知識として持つ前提で、変更検知と対応付ける（[設計 §2.2](docs/features/extension-boundary-and-degraded-submission.md#22-能力と知識を分ける)）。対象要素が見つからない場合を`L1`の縮退運転へ写像し、拡張機能の更新を待たずにAlgoLoom本体の更新で追随できる範囲を明記する。
7. 新しい内容ハッシュを受け入れる保守手順を決める。差分と必須条件の確認、確認記録、利用者へ確認済み版を案内する方法、旧い基準へ戻す場合の扱いを含める。
8. 確認記録へ残す内容と残さない内容を確定する。Cookie、トークン、ソースコード、生のヘッダー、生のHTML、`contest.js`の本文を残さない。
9. AtCoder公式の一般的な正確な頻度は確認できなかったため、[`TD-06`〜`TD-08`](#フェーズ1-公開情報による配布判断)の判断（server指示優先、自動再試行なし、有限通信、release時のmaintainer確認を主とする）と実測へ引き渡す。確定値を工程7で測るまでの安全な既定動作を`spec/`に残す。
10. 決定を対象文書へ反映し、用語、状態、依存関係、受け入れシナリオの意味を一致させる。
